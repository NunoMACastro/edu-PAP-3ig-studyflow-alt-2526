...
const ROLE_JOURNEY_BASE_GZIP_LIMIT_BYTES = 190 * 1024;
// O editor e visualizador GFM seguro são uma capacidade transversal nova para
// aluno e professor. A margem explícita mantém visível o custo da funcionalidade
// sem diluir os limites da entrada pública ou de cada chunk individual.
const MARKDOWN_ROLE_JOURNEY_ALLOWANCE_BYTES = 50 * 1024;
const ROLE_JOURNEY_GZIP_LIMIT_BYTES =
    ROLE_JOURNEY_BASE_GZIP_LIMIT_BYTES +
    MARKDOWN_ROLE_JOURNEY_ALLOWANCE_BYTES;
...
function getAllowedRoles(source) {
    ...
    if (source.includes("/features/student-assistant/")) return ["STUDENT"];
