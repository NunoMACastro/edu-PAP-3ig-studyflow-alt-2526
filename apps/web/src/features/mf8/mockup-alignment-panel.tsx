...
function getStatusClassName(status: MockupAlignmentStatus): string {
    if (status === "VALIDADO") {
        return "bg-studyflow-card text-studyflow-brand";
    }

    if (status === "BLOQUEADO") {
        return "bg-studyflow-card text-studyflow-alert";
    }
    ...
    return (
        <article className="sf-panel space-y-3">
            <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                    <h3 className="font-semibold text-studyflow-text">{item.screen}</h3>
                    <p className="text-sm text-studyflow-text">{item.mockupFocus}</p>
                </div>
                <span
            ...
            <dl className="grid gap-3 text-sm md:grid-cols-3">
                <div>
                    <dt className="font-medium text-studyflow-text">Rota real</dt>
                    <dd>
                ...
                <div>
                    <dt className="font-medium text-studyflow-text">Estado esperado</dt>
                    <dd className="text-studyflow-text">{item.expectedState}</dd>
                </div>
                <div>
                    <dt className="font-medium text-studyflow-text">Evidence</dt>
                    <dd className="text-studyflow-text">{item.evidence}</dd>
                </div>
            </dl>
        </article>
    );
        ...
        <section className="space-y-4" aria-labelledby="mockup-alignment-title">
            <div className="flex flex-wrap items-end justify-between gap-4">
                <div>
                    ...
                    <h2 id="mockup-alignment-title" className="text-xl font-bold text-studyflow-text">
                        Alinhamento ao mockup
                    </h2>
                    <p className="max-w-3xl text-sm text-studyflow-text">
                ...
                <dl className="sf-panel grid min-w-56 grid-cols-2 gap-2 text-sm">
                    <div>
                        <dt className="font-medium text-studyflow-text">Total</dt>
                        <dd className="text-studyflow-text">{summary.total}</dd>
                    </div>
                    <div>
                        <dt className="font-medium text-studyflow-text">Pendentes</dt>
                        <dd className="text-studyflow-text">{summary.pending}</dd>
                    </div>
                    <div>
                        <dt className="font-medium text-studyflow-text">Validados</dt>
                        <dd className="text-studyflow-text">{summary.validated}</dd>
                    </div>
                    <div>
                        <dt className="font-medium text-studyflow-text">Bloqueados</dt>
                        <dd className="text-studyflow-text">{summary.blocked}</dd>
                    </div>
                </dl>
            </div>
