    ...
    return (
        <section className="sf-surface space-y-4">
            ...
            <form className="space-y-3" onSubmit={(event) => void handleSubmit(event)}>
                <label className="block" htmlFor="curriculum-job-ids">
                    Jobs
                    <input id="curriculum-job-ids" value={jobIds} onChange={(event) => setJobIds(event.target.value)} />
                </label>
            ...
            <div className="grid gap-2">
                {response?.topics.map((topic) => (
                    <article className="rounded-xl border border-studyflow-border/10 bg-studyflow-page/35 p-3 text-sm" key={topic.materialId}>
