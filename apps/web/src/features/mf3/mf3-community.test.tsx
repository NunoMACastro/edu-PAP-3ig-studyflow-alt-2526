...
describe("Mf3CommunityPage", () => {
    ...
    it("executa cada fluxo de IA, pesquisa e criação sem misturar contextos", async () => {
        ...
        scope = within(panel("Assistente de estudo"));
    ...
    it("cria grupo, mensagem e sessão e atualiza preferências", async () => {
        ...
        let dialog = await screen.findByRole("dialog", { name: "Criar grupo" });
        await user.type(within(dialog).getByLabelText("Nome"), "Grupo novo");
        await user.type(within(dialog).getByLabelText("Disciplina"), "Matemática");
        await user.type(within(dialog).getByLabelText("Descrição"), "Preparação semanal");
        ...
        await user.click(scope.getByRole("button", { name: "Agendar sessão" }));
        dialog = await screen.findByRole("dialog", { name: "Agendar sessão" });
        await user.type(within(dialog).getByLabelText("Título"), "Sessão nova");
        fireEvent.change(within(dialog).getByLabelText("Início"), { target: { value: "2026-07-12T10:00" } });
        await user.clear(within(dialog).getByLabelText("Minutos"));
        await user.type(within(dialog).getByLabelText("Minutos"), "30");
        await user.type(within(dialog).getByLabelText("Objetivo"), "Rever equações");
        ...
        await user.click(scope.getByRole("checkbox", { name: "na aplicação" }));
