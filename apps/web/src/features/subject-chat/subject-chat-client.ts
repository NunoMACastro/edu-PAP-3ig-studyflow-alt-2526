...
/** Marca como lidas apenas mensagens que o aluno já conseguiu apresentar. */
export function markStudentSubjectChatRead(subjectId: string): Promise<void> {
    return requestMf3Json<void>(`/api/student/subjects/${subjectId}/chat/read`, {
        method: "PUT",
    }).then(() => {
        window.dispatchEvent(new CustomEvent("student-subject-chat-read", {
            detail: { subjectId },
        }));
    });
}
