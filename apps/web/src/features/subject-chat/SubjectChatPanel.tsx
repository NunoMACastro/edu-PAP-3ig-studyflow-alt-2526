...
export function SubjectChatPanel({ subjectId, role, readOnly = false }: SubjectChatPanelProps) {
    ...
    useEffect(() => {
        ...
        /**
         * Obtém primeiro o histórico; o WebSocket é apenas uma melhoria para escrita ativa.
         */
        async function reconcileHistory(): Promise<void> {
            try {
                ...
                if (role === "STUDENT") {
                    void markStudentSubjectChatRead(subjectId).catch(() => undefined);
                }
            } catch (caught) {
                if (!active) return;
                setError(
        ...
        socket.on("subject-chat:message", (message: SubjectChatMessage) => {
            ...
            if (role === "STUDENT" && message.authorRole === "TEACHER") {
                void markStudentSubjectChatRead(subjectId).catch(() => undefined);
            }
        });
    ...
    return (
        <Surface as="section" className="space-y-4" variant="elevated">
            ...
            {loading ? (
                <InlineNotice>A carregar histórico...</InlineNotice>
            ) : null}
            ...
            {!readOnly ? <form className="sticky bottom-3 space-y-3 rounded-2xl border border-studyflow-border/10 bg-studyflow-card/95 p-3 shadow-2xl backdrop-blur-xl" onSubmit={(event) => void handleSubmit(event)}>
            ...
            </form> : <InlineNotice>Esta disciplina está arquivada. Podes consultar as mensagens anteriores, mas já não podes enviar novas mensagens.</InlineNotice>}
        </Surface>
    );
}
