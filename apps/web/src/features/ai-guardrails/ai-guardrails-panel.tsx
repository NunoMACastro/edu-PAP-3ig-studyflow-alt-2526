    ...
    return (
        <section className="sf-surface space-y-4">
            ...
            <form className="space-y-3" onSubmit={(event) => void handleSubmit(event)}>
                <label className="block" htmlFor="ai-guardrails-context">
                    {t(messageKeys.guardrailsContextLabel)}
                    <select
                        id="ai-guardrails-context"
                        value={contextType}
                        onChange={(event) =>
                ...
                <label className="block" htmlFor="ai-guardrails-resource">
                    {t(messageKeys.guardrailsResourceLabel)}
                    <input id="ai-guardrails-resource" value={resourceId} onChange={(event) => setResourceId(event.target.value)} />
                </label>
                <label className="block" htmlFor="ai-guardrails-prompt">
                    {t(messageKeys.guardrailsPromptLabel)}
                    <textarea id="ai-guardrails-prompt" rows={3} value={prompt} onChange={(event) => setPrompt(event.target.value)} />
                </label>
            ...
            {decision ? (
                <div className="rounded-xl border border-studyflow-border/10 bg-studyflow-page/35 p-3 text-sm">
                    <p className={decision.allowed ? "text-studyflow-brandText" : "text-studyflow-alertText"}>
                        {decision.allowed
