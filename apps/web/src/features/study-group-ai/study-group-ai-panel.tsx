 ...
 */
export function StudyGroupAiPanel({ initialGroupId, contextLocked = false }: StudyGroupAiPanelProps) {
        ...
        const nextAnswer = await submitAction.run(
            ...
            () =>
                askStudyGroupAi(groupId, {
                    question,
                    sourceShareIds: contextLocked ? undefined : sourceShareIds
                        .split(",")
    ...
    return (
        <section className="sf-surface space-y-4">
            <h2 className="text-lg font-semibold">Assistente de estudo</h2>
            {submitAction.error ? (
                <p className="sf-error" role="alert">{submitAction.error}</p>
            ) : null}
            <form className="space-y-3" onSubmit={(event) => void handleSubmit(event)}>
                {!contextLocked ? <label className="block" htmlFor="study-group-ai-group">
                    Grupo
                    <input id="study-group-ai-group" value={groupId} onChange={(event) => setGroupId(event.target.value)} />
                </label> : null}
                {!contextLocked ? <label className="block" htmlFor="study-group-ai-sources">
                    Fontes
                    <input id="study-group-ai-sources" value={sourceShareIds} onChange={(event) => setSourceShareIds(event.target.value)} />
                </label> : <p className="text-sm text-studyflow-text/65">O Assistente usa automaticamente as fontes autorizadas deste grupo.</p>}
                <label className="block" htmlFor="study-group-ai-question">
                    Pergunta
                    <textarea id="study-group-ai-question" rows={3} value={question} onChange={(event) => setQuestion(event.target.value)} />
                </label>
