...
export function EmptyState({ action, compact = false, description, icon = "info", title }: {
    action?: ReactNode;
    compact?: boolean;
    description?: string;
    icon?: ShellIconName;
    ...
    if (compact) {
        return (
            <div
                className="flex min-w-0 items-start gap-3 rounded-xl border border-dashed border-studyflow-border/15 bg-studyflow-card/25 p-4"
                aria-live="polite"
            >
                <span className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-studyflow-brand/10 text-studyflow-brandText">
                    <ShellIcon className="h-5 w-5" name={icon} />
                </span>
                <div className="min-w-0">
                    <h2 className="font-bold tracking-tight">{title}</h2>
                    {description ? (
                        <p className="mt-1 text-sm leading-6 text-studyflow-text/65">{description}</p>
                    ) : null}
...
export function StatusBadge({ children, className = "", tone = "neutral" }: { children: ReactNode; className?: string; tone?: CalmTone }) {
