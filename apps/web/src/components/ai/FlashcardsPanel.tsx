...
export function FlashcardsPanel({ artifact }: FlashcardsPanelProps) {
    ...
    if (cards.length === 0) {
        return (
            <section className="sf-surface" aria-label="Flashcards">
    ...
    return (
        <section className="sf-surface space-y-4" aria-label="Flashcards">
            <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <p className="rounded-full bg-studyflow-page px-3 py-1 text-sm font-semibold text-studyflow-brandText">
                    {progressLabel}
                </p>
            </div>
            ...
            {practice.completed ? (
                <div className="rounded-md border border-studyflow-brand bg-studyflow-card p-4" role="status">
                    <p className="font-semibold text-studyflow-brandText">Sessão concluída.</p>
                    <p className="mt-1 text-sm text-studyflow-brandText">
                ...
                <article className="rounded-xl border border-studyflow-border/10 bg-studyflow-page/35 bg-studyflow-card p-4">
                    <p className="text-xs font-semibold uppercase text-studyflow-brandText">
                        Pergunta
                    </p>
                    <h3 className="mt-1 text-lg font-bold text-studyflow-text">
                        {currentCard.front}
                    </h3>

                    {practice.answerVisible ? (
                        <div
                            className="mt-4 rounded-xl border border-studyflow-border/10 bg-studyflow-page/45 p-4"
                            aria-live="polite"
                        >
                            <p className="text-xs font-semibold uppercase text-studyflow-text">
                                Resposta
                            </p>
                            <p className="mt-1 text-studyflow-text">{currentCard.back}</p>
                        </div>
                    ) : (
                        <p
                            className="mt-4 rounded-md bg-studyflow-card p-4 text-sm text-studyflow-text"
                            aria-live="polite"
                        >
