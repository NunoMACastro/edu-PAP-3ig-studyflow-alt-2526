...
type QuizPanelProps = {
    ...
    studyAreaId?: string;
    canAttempt?: boolean;
    submitAnswers?: (answers: number[]) => Promise<QuizAttemptResult>;
};
...
export function QuizPanel({
    ...
    async function handleSubmit(event: FormEvent): Promise<void> {
        ...
        try {
            if (!submitAnswers && !studyAreaId) {
                throw new Error("Não foi possível identificar o destino do quiz.");
            }
            setAttempt(
                submitAnswers
        ...
        <form className="space-y-4" onSubmit={(event) => void handleSubmit(event)}>
            {attempt ? (
                <div className="sf-surface">
                    <p className="text-sm font-semibold text-studyflow-brandText">
            ...
            {questions.map((question, index) => {
                ...
                return (
                <article className="sf-list-card space-y-3" key={index}>
                    <h2 className="font-semibold">{question.question}</h2>
                    <div className="space-y-2">
                        {question.options.map((option, optionIndex) => (
                            <label
                                className="flex items-center gap-3 rounded-xl border border-studyflow-border/10 bg-studyflow-page/35 px-3 py-2 text-sm"
                                key={optionIndex}
                            >
                                <input
                                    checked={answers[index] === optionIndex}
                                    disabled={!canAttempt || Boolean(attempt) || isSubmitting}
                    ...
                    {result ? (
                        <>
                            <p
                                className={
                                    result.isCorrect
                                        ? "text-sm text-studyflow-brandText"
                                        : "text-sm text-studyflow-alertText"
                                }
                            >
            ...
            {!canAttempt ? (
                <p className="sf-notice sf-notice-attention" role="status">
                    Este material está em arquivo e já não permite novas tentativas.
                </p>
            ) : null}
            {!attempt && canAttempt ? (
                <button
