...
describe("AppShell", () => {
    ...
    it("abre e fecha o menu de conta do aluno por Escape e clique exterior", async () => {
        const user = userEvent.setup();
        render(
        ...
        const toggle = screen.getByRole("button", { name: "Conta: student@example.test" });
        await user.click(toggle);
        expect(screen.getByRole("menu")).toBeTruthy();
        expect(screen.getByRole("menuitem", { name: "Perfil" })).toBeTruthy();
...
describe("contrato de navegação por papel", () => {
    it("resolve home, itens e estados ativos para todos os papéis", () => {
        ...
        expect(isNavigationItemActive(adminHome, "/app")).toBe(true);
        expect(isNavigationItemActive(studentHome, "/app/areas")).toBe(false);
        expect(isNavigationItemActive(getNavigationForRole("STUDENT")[1], "/app/areas/area-id")).toBe(true);
    });
});
