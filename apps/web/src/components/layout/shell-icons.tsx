 ...
 *
 * @param props Texto visível e alinhamento relativo ao ícone.
 * @returns Tooltip acessível por hover/focus sem depender do `title` nativo.
 */
export function IconTooltip({ align = "right", children }: IconTooltipProps) {
    const positionClass =
        align === "center"
    ...
    return (
        <span
            aria-hidden="true"
            className={`pointer-events-none absolute top-full z-40 mt-2 whitespace-nowrap rounded-md border border-studyflow-border bg-studyflow-card px-2 py-1 text-xs font-semibold text-studyflow-text opacity-0 shadow-lg transition group-hover:opacity-100 group-focus-visible:opacity-100 ${positionClass}`}
        >
            {children}
        </span>
    );
