...
export function WorkspaceTabs({ items }: { items: { label: string; href: string; active: boolean }[] }) {
    return <nav aria-label="Secções do workspace" className="flex gap-1 overflow-x-auto border-b border-studyflow-border/10">{items.map((item) => <a aria-current={item.active ? "page" : undefined} className={item.active ? "min-h-11 shrink-0 border-b-2 border-studyflow-brand px-3 py-3 text-sm font-semibold text-studyflow-brandText" : "min-h-11 shrink-0 px-3 py-3 text-sm font-semibold text-studyflow-text/60 hover:text-studyflow-text"} href={item.href} key={item.href}>{item.label}</a>)}</nav>;
}
