...
export function ProtectedRoutes({ user, onLogout }: ProtectedRoutesProps) {
    ...
    return (
        <ProtectedLayout user={user} onLogout={onLogout}>
            <Suspense
                fallback={
                    <p aria-live="polite" className="sf-panel text-sm">
                        A carregar página...
                    </p>
                }
            >
...
export function ForbiddenPage() {
    return (
        <section className="sf-panel max-w-2xl space-y-3" role="alert">
            <p className="text-sm font-semibold text-studyflow-alertText">Erro 403</p>
            <h1 className="text-xl font-bold">Acesso não permitido</h1>
            <p className="text-sm text-studyflow-text">
                Esta área não está disponível para o teu perfil. A API continuará
                a validar todas as permissões do recurso.
            </p>
        </section>
    );
}
...
export function NotFoundPage() {
    return (
        <section className="sf-panel max-w-2xl space-y-3">
            <p className="text-sm font-semibold text-studyflow-alertText">Erro 404</p>
            <h1 className="text-xl font-bold">Página não encontrada</h1>
            <p className="text-sm text-studyflow-text">
                Confirma o endereço ou regressa através da navegação principal.
            </p>
        </section>
    );
}
