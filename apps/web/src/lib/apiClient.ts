...
export type StudentAction = {
    key: string;
    kind: "SUBJECT" | "STUDY_AREA" | "GUIDED_ROOM" | "STUDY_ROOM" | "STUDY_GROUP" | "OFFICIAL_TEST" | "CLASS_PROJECT" | "ROUTINE" | "GOAL" | "GROUP_SESSION" | "ROOM_SESSION" | "SUBJECT_CHAT";
    title: string;
    contextLabel?: string;
    dueAt?: string;
    urgency: "OVERDUE" | "TODAY" | "UPCOMING" | "AVAILABLE";
    targetPath: string;
    contextMeta?: {
...
export type StudentSubjectOverview = {
    subject: { id: string; name: string; code: string; description?: string; classId: string; className: string };
    readOnly: boolean;
    counts: { materials: number; tests: number; approvedContent: number; unreadChat: number };
    recentMaterial: { _id: string; title: string } | null;
    nextTest: { _id: string; title: string } | null;
};
...
export type StudyMaterial = {
    _id: string;
    title: string;
    type: "PDF" | "DOCX" | "URL" | "TOPIC" | "MARKDOWN";
...
export type OfficialMaterial = {
    ...
    teacherId: string;
    title: string;
    type: "TEXT" | "URL" | "PDF" | "DOCX" | "MARKDOWN";
    status: "PROCESSED" | "REFERENCE_ONLY" | "PENDING_PROCESSING" | "DRAFT";
    textContent?: string;
    markdownSource?: string;
    sourceUrl?: string;
    originalName?: string;
...
export type StudentClassProject = Omit<ClassProject, "teacherId"> & {
    readOnly: boolean;
    myProgress: "NOT_STARTED" | "IN_PROGRESS" | "COMPLETED";
    completedAt?: string;
};
...
function parseStudentOfficialMaterial(value: unknown): StudentOfficialMaterial {
    ...
    if (!["TEXT", "URL", "PDF", "DOCX", "MARKDOWN"].includes(String(object.type))) {
    ...
    if (object.revisionInfo !== undefined) {
        const revision = contractObject(object.revisionInfo, "material.revisionInfo");
        contractFiniteNumber(revision.revision, "material.revisionInfo.revision");
        contractString(revision.updatedAt, "material.revisionInfo.updatedAt");
        if (revision.changeSummary !== undefined) {
            contractString(revision.changeSummary, "material.revisionInfo.changeSummary");
        }
    }
    if ("teacherId" in object) {
...
function parseStudentClassProject(value: unknown): StudentClassProject {
    ...
    if (!["NOT_STARTED", "IN_PROGRESS", "COMPLETED"].includes(String(object.myProgress))) {
        throw new TypeError("Projeto sem progresso pessoal válido.");
    }
    if ("teacherId" in object) {
...
function parseApprovedAiContent(value: unknown): ApprovedAiContent {
    ...
    if (object.origin !== undefined && object.origin !== "TEACHER_AUTHORED") {
        throw new TypeError("Conteúdo aprovado sem origem docente válida.");
    }
    const canAttempt = object.canAttempt === undefined
        ? true
        : contractBoolean(object.canAttempt, "conteúdo.canAttempt");
    const unavailableReason = object.unavailableReason === undefined
        ? undefined
        : contractString(object.unavailableReason, "conteúdo.unavailableReason") as ApprovedAiContentBase["unavailableReason"];
...
export async function getProfile(): Promise<StudentProfile | null> {
    const profile = await requestJson<StudentProfile | null | undefined>(
...
/** Obtém a página Hoje já agregada e autorizada no servidor. */
export function getStudentToday(): Promise<StudentTodayState> {
    return requestJson<StudentTodayState>("/api/student/today");
}

/** Regista continuidade apenas depois de um workspace autorizado carregar. */
export function rememberStudentContext(input: {
    kind: "SUBJECT" | "STUDY_AREA" | "GUIDED_ROOM" | "STUDY_ROOM" | "STUDY_GROUP" | "OFFICIAL_TEST" | "CLASS_PROJECT";
    contextId: string;
}): Promise<StudentAction> {
    return requestJson<StudentAction>("/api/students/me/recent-context", {
        method: "PUT",
        body: JSON.stringify(input),
    });
}

export function getStudentSubjectOverview(subjectId: string): Promise<StudentSubjectOverview> {
    return requestJson<StudentSubjectOverview>(`/api/student/subjects/${subjectId}/overview`);
}

export function listStudentSubjectChatUnread(): Promise<Array<{
    subjectId: string;
    unreadCount: number;
    lastMessageAt?: string;
}>> {
    return requestJson("/api/student/subject-chat/unread");
}

export function searchStudentStudies(input: {
    query: string;
    scope: { type: "SUBJECT" | "STUDY_AREA"; id: string } | { type: "ALL_STUDIES" };
}): Promise<{ query: string; results: StudentSearchResult[] }> {
    return requestJson("/api/student/search", {
        method: "POST",
        body: JSON.stringify(input),
    });
}
...
/** Obtém o detalhe autorizado de um material privado. */
export function getPrivateMaterial(
    studyAreaId: string,
    materialId: string,
): Promise<StudyMaterial> {
    return requestJson<StudyMaterial>(
        `/api/study-areas/${studyAreaId}/materials/${materialId}`,
    );
}

/** Guarda uma revisão Markdown privada com precondição otimista. */
export function updatePrivateMarkdown(
    studyAreaId: string,
    materialId: string,
    input: { title?: string; markdownSource: string; expectedRevision: number },
): Promise<StudyMaterial> {
    return requestJson<StudyMaterial>(
        `/api/study-areas/${studyAreaId}/materials/${materialId}/markdown`,
...
/** Lê uma sala concreta depois de o backend confirmar membership. */
export function getStudyRoom(roomId: string): Promise<StudyRoom> {
    return requestJson<StudyRoom>(`/api/study-rooms/${roomId}`);
}
...
/** Obtém o detalhe docente, incluindo fontes Markdown em rascunho. */
export function getTeacherOfficialMaterial(
    subjectId: string,
    materialId: string,
): Promise<OfficialMaterial> {
    return requestJson<OfficialMaterial>(
        `/api/teacher/subjects/${subjectId}/materials/${materialId}`,
    );
}

/** Guarda Markdown oficial sem sobrescrever uma revisão concorrente. */
export function updateOfficialMarkdown(
    subjectId: string,
    materialId: string,
    input: { title?: string; markdownSource: string; expectedRevision: number },
): Promise<OfficialMaterial> {
    return requestJson<OfficialMaterial>(
        `/api/teacher/subjects/${subjectId}/materials/${materialId}/markdown`,
        { method: "PATCH", body: JSON.stringify(input) },
    );
}

/** Publica pela primeira vez o rascunho Markdown atual. */
export function publishOfficialMarkdown(
    subjectId: string,
    materialId: string,
    expectedRevision: number,
): Promise<OfficialMaterial> {
    return requestJson<OfficialMaterial>(
        `/api/teacher/subjects/${subjectId}/materials/${materialId}/publish`,
...
export function getStudentClassProject(projectId: string): Promise<StudentClassProject> {
    return requestContract(`/api/student/class-projects/${projectId}`, parseStudentClassProject);
}

export function updateStudentClassProjectProgress(
    projectId: string,
    status: StudentClassProject["myProgress"],
): Promise<StudentClassProject> {
    return requestContract(
        `/api/student/class-projects/${projectId}/progress`,
        parseStudentClassProject,
        { method: "PATCH", body: JSON.stringify({ status }) },
    );
}
...
/** Contextos suportados pelo Assistente conversacional do aluno. */
...
export type StudentAssistantConversation = {
    id: string;
    title: string;
    status: "DRAFT" | "ACTIVE" | "ARCHIVED";
    origin: "NATIVE" | "LEGACY_MIGRATION" | "LEGACY_API" | "FORK";
    fork?: {
    ...
    readOnly: boolean;
    readOnlyReason?: "LEGACY_MIGRATION" | "ACCESS_REVOKED";
    capabilities: {
        canInviteFork: boolean;
...
export type StudentAssistantTurn = {
    id: string;
    question: string;
    answer: string;
    citations: Array<{
        label: string;
        kind: "OFFICIAL_MATERIAL" | "PRIVATE_MATERIAL" | "GROUP_RESOURCE" | "ROOM_SHARE";
        targetPath?: string;
    }>;
    createdAt: string;
    visibility?: "PRIVATE" | "SHARED";
...
export type StudentAssistantForkRecipient = {
    id: string;
    email: string;
};

export type StudentAssistantForkInvitation = {
    id: string;
    ...
    sender: StudentAssistantForkRecipient;
    recipient: StudentAssistantForkRecipient;
    turnCount: number;
    expiresAt: string;
    createdAt?: string;
};
...
export type StudentAssistantArtifact = {
    id: string;
    ...
    targetPath: string;
    target: {
        kind: StudentStudyMaterialTargetKind;
        id: string;
        label: string;
...
export type StudentStudyMaterialTargetKind = "STUDY_AREA" | "SUBJECT" | "CLASS";

export type StudentStudyMaterialTarget = {
    kind: StudentStudyMaterialTargetKind;
    id: string;
    label: string;
    secondaryLabel?: string;
...
export type StudentAssistantArtifactSetup = {
    canCreate: true;
    targetMode: "FIXED" | "SELECTION_REQUIRED";
    fixedTarget?: StudentStudyMaterialTarget;
    preview: {
        turnCount: number;
        candidateSourceCount: number;
        groundingMode: "CHAT_AND_SOURCES" | "CHAT_ONLY";
        sourceLimits: { SUMMARY: number; STUDY_TOOL: number };
    };
};

export type StudentStudyMaterialDetail = StudentAssistantArtifact & {
...
export type StudentAssistantArtifactGenerationResult =
    | { status: "DONE"; artifact: StudentAssistantArtifact }
    | {
          status: "QUEUED" | "PROCESSING" | "FAILED";
          job: StudentAssistantArtifactJob;
      };

export function listStudentAssistantContexts(input: {
    query?: string;
    cursor?: string;
    limit?: number;
} = {}): Promise<{ items: StudentAssistantContext[]; nextCursor: string | null }> {
    const params = new URLSearchParams();
    if (input.query) params.set("query", input.query);
    if (input.cursor) params.set("cursor", input.cursor);
    if (input.limit) params.set("limit", String(input.limit));
    return requestJson(`/api/student/assistant/contexts${params.size ? `?${params}` : ""}`, {
        cache: "no-store",
    });
}

export function getStudentAssistantContext(
    kind: StudentAssistantContextKind,
    contextId: string,
): Promise<StudentAssistantContext> {
    return requestJson(`/api/student/assistant/contexts/${kind}/${contextId}`, {
        cache: "no-store",
    });
}

export function listStudentAssistantConversations(input: {
    cursor?: string;
    limit?: number;
    contextKind?: StudentAssistantContextKind;
    contextId?: string;
    status?: "ACTIVE" | "ARCHIVED";
} = {}): Promise<{ items: StudentAssistantConversation[]; nextCursor: string | null }> {
    const params = new URLSearchParams();
    if (input.cursor) params.set("cursor", input.cursor);
    if (input.limit) params.set("limit", String(input.limit));
    if (input.contextKind) params.set("contextKind", input.contextKind);
    if (input.contextId) params.set("contextId", input.contextId);
    if (input.status) params.set("status", input.status);
    return requestJson(`/api/student/assistant/conversations${params.size ? `?${params}` : ""}`, {
        cache: "no-store",
    });
}

export function createStudentAssistantConversation(context: {
    kind: StudentAssistantContextKind;
    id: string;
}): Promise<StudentAssistantConversation> {
    return requestJson("/api/student/assistant/conversations", {
        method: "POST",
        body: JSON.stringify({ context }),
    });
}

export function listStudentAssistantForkRecipients(
    conversationId: string,
    input: { query?: string; cursor?: string; limit?: number } = {},
): Promise<{ items: StudentAssistantForkRecipient[]; nextCursor: string | null }> {
    const params = new URLSearchParams();
    if (input.query) params.set("query", input.query);
    if (input.cursor) params.set("cursor", input.cursor);
    if (input.limit) params.set("limit", String(input.limit));
    return requestJson(
        `/api/student/assistant/conversations/${conversationId}/fork-recipients${params.size ? `?${params}` : ""}`,
        { cache: "no-store" },
    );
}

export function createStudentAssistantForkInvitation(
    conversationId: string,
    recipientId: string,
): Promise<StudentAssistantForkInvitation> {
    return requestJson(
        `/api/student/assistant/conversations/${conversationId}/fork-invitations`,
        {
            method: "POST",
...
export function listStudentAssistantForkInvitations(input: {
    direction: "received" | "sent";
    conversationId?: string;
    cursor?: string;
    limit?: number;
} = { direction: "received" }): Promise<{ items: StudentAssistantForkInvitation[]; nextCursor: string | null }> {
    const direction = input.direction === "sent" ? "sent" : "received";
    const params = new URLSearchParams({ direction });
    if (input.conversationId) params.set("conversationId", input.conversationId);
    if (input.cursor) params.set("cursor", input.cursor);
    if (input.limit) params.set("limit", String(input.limit));
    return requestJson(`/api/student/assistant/fork-invitations?${params}`, {
        cache: "no-store",
    });
}

export function acceptStudentAssistantForkInvitation(
    invitationId: string,
): Promise<StudentAssistantConversation> {
    return requestJson(`/api/student/assistant/fork-invitations/${invitationId}/accept`, {
        method: "POST",
    });
}

export function declineStudentAssistantForkInvitation(
    invitationId: string,
): Promise<{ id: string; status: "DECLINED" }> {
    return requestJson(`/api/student/assistant/fork-invitations/${invitationId}/decline`, {
        method: "POST",
    });
}

export function cancelStudentAssistantForkInvitation(
    invitationId: string,
): Promise<{ id: string; status: "CANCELLED" }> {
    return requestJson(`/api/student/assistant/fork-invitations/${invitationId}`, {
        method: "DELETE",
    });
}

export function getStudentAssistantConversation(
    conversationId: string,
): Promise<StudentAssistantConversation> {
    return requestJson(`/api/student/assistant/conversations/${conversationId}`, {
        cache: "no-store",
    });
}

export function updateStudentAssistantConversation(
    conversationId: string,
...
export function deleteStudentAssistantConversation(
    conversationId: string,
): Promise<{
    deleted: true;
    retainedTurnCount: number;
    retentionReasons: Array<"SHARED" | "FORK_REFERENCE" | "SUPERVISED">;
...
export function listStudentAssistantMessages(
    conversationId: string,
    input: { before?: string; limit?: number } = {},
): Promise<{ items: StudentAssistantTurn[]; previousCursor: string | null }> {
    const params = new URLSearchParams();
    if (input.before) params.set("before", input.before);
    if (input.limit) params.set("limit", String(input.limit));
    return requestJson(
        `/api/student/assistant/conversations/${conversationId}/messages${params.size ? `?${params}` : ""}`,
        { cache: "no-store" },
    );
}

export function askStudentAssistant(
    conversationId: string,
    question: string,
): Promise<StudentAssistantTurn> {
    return requestJson(`/api/student/assistant/conversations/${conversationId}/messages`, {
        method: "POST",
        body: JSON.stringify({ question }),
    });
}

export function listStudentAssistantArtifacts(
    conversationId: string,
    input: { before?: string; limit?: number } = {},
): Promise<{ items: StudentAssistantArtifact[]; previousCursor: string | null }> {
    const params = new URLSearchParams();
    if (input.before) params.set("before", input.before);
    if (input.limit) params.set("limit", String(input.limit));
    return requestJson(
        `/api/student/assistant/conversations/${conversationId}/artifacts${params.size ? `?${params}` : ""}`,
        { cache: "no-store" },
    );
}

export function getStudentAssistantArtifactSetup(
    conversationId: string,
): Promise<StudentAssistantArtifactSetup> {
    return requestJson(
        `/api/student/assistant/conversations/${conversationId}/artifact-setup`,
        { cache: "no-store" },
    );
}

export function listStudentAssistantArtifactTargets(
    conversationId: string,
    input: { query?: string; cursor?: string; limit?: number } = {},
): Promise<{ items: StudentStudyMaterialTarget[]; nextCursor: string | null }> {
    const params = new URLSearchParams();
    if (input.query) params.set("query", input.query);
    if (input.cursor) params.set("cursor", input.cursor);
    if (input.limit) params.set("limit", String(input.limit));
    return requestJson(
        `/api/student/assistant/conversations/${conversationId}/artifact-targets${params.size ? `?${params}` : ""}`,
        { cache: "no-store" },
    );
}

export function generateStudentAssistantArtifact(
    conversationId: string,
    input: {
        type: StudentAssistantArtifactType;
        topic?: string;
        target?: { kind: StudentStudyMaterialTargetKind; id: string };
    },
    idempotencyKey: string,
): Promise<StudentAssistantArtifactGenerationResult> {
    return requestJson(
        `/api/student/assistant/conversations/${conversationId}/artifacts`,
        {
            method: "POST",
...
export function listStudentStudyMaterials(input: {
    ...
    cursor?: string;
    limit?: number;
} = {}): Promise<{ items: StudentAssistantArtifact[]; nextCursor: string | null }> {
    const params = new URLSearchParams();
    ...
    if (input.limit) params.set("limit", String(input.limit));
    return requestJson(
        `/api/student/study-materials${params.size ? `?${params}` : ""}`,
        { cache: "no-store" },
    );
}

export function getStudentStudyMaterial(
    artifactId: string,
): Promise<StudentStudyMaterialDetail> {
    return requestJson(`/api/student/study-materials/${artifactId}`, {
        cache: "no-store",
    });
}

export function deleteStudentStudyMaterial(
    artifactId: string,
...
export function submitStudentStudyMaterialQuizAttempt(
    artifactId: string,
    answers: number[],
): Promise<QuizAttemptResult> {
    return requestJson(`/api/student/study-materials/${artifactId}/quiz-attempts`, {
        method: "POST",
        body: JSON.stringify({ answers }),
    });
}

export async function exportStudentStudyMaterial(
    artifactId: string,
    format: ArtifactExportFormat,
): Promise<ArtifactExportFile> {
    const query = new URLSearchParams({ format });
    const response = await requestApiResponse(
        `/api/student/study-materials/${encodeURIComponent(artifactId)}/export?${query}`,
        { method: "GET" },
    );
    return {
        fileName:
...
export function listStudentAssistantArtifactJobs(
    conversationId: string,
    input: { status?: "ACTIVE"; limit?: number } = {},
): Promise<StudentAssistantArtifactJob[]> {
    const params = new URLSearchParams();
    if (input.status) params.set("status", input.status);
    if (input.limit) params.set("limit", String(input.limit));
    return requestJson(
        `/api/student/assistant/conversations/${conversationId}/artifact-jobs${params.size ? `?${params}` : ""}`,
        { cache: "no-store" },
    );
}

export function getStudentAssistantArtifactJob(
    conversationId: string,
    jobId: string,
    signal?: AbortSignal,
): Promise<StudentAssistantArtifactJob> {
    return requestJson(
        `/api/student/assistant/conversations/${conversationId}/artifact-jobs/${jobId}`,
