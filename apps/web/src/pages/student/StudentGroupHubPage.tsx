...
export function StudentGroupHubPage() {
    ...
    async function refresh(): Promise<void> {
        const [guidedPage, nextGroups, nextRooms] = await Promise.all([
            ...
            {!loading && !error ? (
                <>
                    ...
                    {!view || view === "grupos" ? <GroupSection title="Grupos" empty="Ainda não participas em grupos." action={<button className="sf-button-secondary" onClick={() => { resetForm(); setGroupOpen(true); }} type="button">Novo grupo</button>}>
                        {groups.map((group) => <ContextCard actionLabel="Abrir grupo" description={group.disciplineName ?? group.description} href={`/app/grupos/${group._id}`} key={group._id} title={group.title} />)}
                    </GroupSection> : null}
