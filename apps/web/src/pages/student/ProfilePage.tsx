...
const canonicalYears = [...Array.from({ length: 12 }, (_, index) => `${index + 1}.º ano`), "Ensino superior", "Prefiro não indicar"];
                ...
                <div className="grid gap-4 md:grid-cols-2">
                    <FormField id="student-profile-year" label="Ano" helpText="Ajuda a IA da sala a adaptar a linguagem e o detalhe das explicações.">
                        ...
                        >
                            ...
                            {profile.year && !canonicalYears.includes(profile.year) ? <option value={profile.year}>{profile.year} (valor atual)</option> : null}
                            {canonicalYears.map((year) => <option key={year} value={year}>{year}</option>)}
                        </select>
                    </FormField>
