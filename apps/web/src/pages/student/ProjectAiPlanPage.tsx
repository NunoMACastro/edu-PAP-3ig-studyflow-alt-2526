...
export function ProjectAiPlanPage({ projectId }: { projectId: string }) {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        setLoadError(null);
        listProjectAiPlans(projectId, { limit: 20 })
            .then((page) => {
                if (!active) return;
                setPlans(page.items);
    ...
    return (
        <section className="space-y-6">
            <PageHeader description="Transforma o teu objetivo num percurso gradual para desenvolver o projeto." title="Plano IA do projeto" />
            <AiConsentGate description="Aceita o tratamento PROJECT_AI para gerar planos de projeto." purpose="PROJECT_AI">
