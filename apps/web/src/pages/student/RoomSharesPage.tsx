            ...
            <div className="space-y-4">
                ...
                <form className="sf-panel space-y-3" onSubmit={(event) => void handleMemberSubmit(event)}>
                    ...
                    <label className="block space-y-2">
                        <span>Email do novo membro</span>
                ...
                {shares.map((share) => (
                    <article className="sf-panel space-y-1" key={share._id}>
                        <h2 className="font-semibold">
                            {share.tombstoned ? "Partilha removida" : share.title}
                        </h2>
                        <p className="text-sm text-studyflow-text">{shareTypeLabel(share.type)} · {share.usableByAi ? "pode alimentar IA" : "referência"}</p>
                        {share.textContent ? <p className="whitespace-pre-wrap text-sm text-studyflow-text">{share.textContent}</p> : null}
                        {share.url ? <a className="text-sm text-studyflow-brandText underline" href={share.url}>{share.url}</a> : null}
                    </article>
                ))}
            </div>
        </section>
