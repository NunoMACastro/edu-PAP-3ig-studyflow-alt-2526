 ...
 */
export function RoomSharesPage({ roomId, embedded = false, mode = "all" }: RoomSharesPageProps) {
    const [shares, setShares] = useState<RoomShare[]>([]);
    const [type, setType] = useState<"NOTE" | "URL" | "MATERIAL_REF">(
        mode === "notes" ? "NOTE" : "NOTE",
    );
    ...
    useEffect(() => {
        ...
        setLoading(true);
        setError(null);
        Promise.all([refresh(), mode === "all" ? loadMaterialOptions() : Promise.resolve()])
    ...
    const visibleShares = mode === "notes"
        ? shares.filter((share) => share.type === "NOTE")
        : shares;
    const openShareButton = (
        <button
            aria-expanded={shareOpen}
            className="sf-button-primary"
            onClick={() => { setError(null); setMemberOpen(false); setShareOpen(true); }}
            type="button"
        >
            {mode === "notes" ? "Nova nota" : "Nova partilha"}
        </button>
    );
    ...
    return (
        <section className="space-y-6">
            {embedded ? (
                <SectionHeader
                    action={openShareButton}
                    description={mode === "notes" ? "Registos persistentes que também podem apoiar o Assistente." : "Notas, ligações e materiais disponíveis aos membros da sala."}
                    title={mode === "notes" ? "Notas" : "Partilhas"}
                />
            ) : (
                <PageHeader
                    action={<>{openShareButton}<button aria-expanded={memberOpen} className="sf-button-secondary" onClick={() => { setError(null); setShareOpen(false); setMemberOpen(true); }} type="button">Adicionar membro</button></>}
                    description="Consulta e adiciona fontes partilhadas com os membros desta sala de estudo."
                    title="Partilhas da sala"
                />
            )}
            <AsyncStateBlock error={error && !shareOpen && !memberOpen ? error : undefined} isEmpty={visibleShares.length === 0} isLoading={loading} emptyMessage={mode === "notes" ? "Ainda não há notas nesta sala" : "Ainda não há partilhas nesta sala"}>
                <div aria-label={mode === "notes" ? "Notas da sala" : "Partilhas da sala"} className="grid gap-3 md:grid-cols-2">
            ...
            <SidePanel closeDisabled={savingShare} description={mode === "notes" ? "Regista uma nota persistente para os membros da sala." : "Adiciona uma nota, ligação ou referência a material próprio."} onClose={() => setShareOpen(false)} open={shareOpen} title={mode === "notes" ? "Nova nota" : "Nova partilha"}>
                <form className="space-y-4" onSubmit={(event) => void handleShareSubmit(event)}>
                    {error ? <p className="sf-error" role="alert">{error}</p> : null}
                    {mode === "all" ? <FormField id="room-share-type" label="Tipo">
            ...
            {!embedded ? <SidePanel closeDisabled={savingMember} description="Convida outro aluno através do email da respetiva conta." onClose={() => setMemberOpen(false)} open={memberOpen} title="Adicionar membro">
