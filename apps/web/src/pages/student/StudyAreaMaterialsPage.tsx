...
import {
    MaterialSubmitForm,
    type MaterialMode,
} from "../../components/materials/MaterialSubmitForm.js";
...
export function StudyAreaMaterialsPage({ studyAreaId }: StudyAreaMaterialsPageProps) {
    ...
    const [materialMode, setMaterialMode] = useState<MaterialMode>("TOPIC");
    ...
    return (
        <section className="space-y-6">
            ...
            <div className="flex flex-wrap gap-2"><button aria-expanded={createOpen} className="sf-button-primary" onClick={() => { setMaterialMode("TOPIC"); setCreateOpen(true); }} type="button">Novo material</button><button aria-expanded={importOpen} className="sf-button-secondary" onClick={() => setImportOpen(true)} type="button">Importar link</button></div>

            {materialList}
            <SidePanel closeDisabled={isSubmitting} description="Adiciona um tópico, URL ou ficheiro à área privada." onClose={() => setCreateOpen(false)} open={createOpen} size={materialMode === "MARKDOWN" ? "wide" : "default"} title="Novo material">
                <MaterialSubmitForm
                    ...
                    onSubmitted={async (created) => {
                        ...
                        if (created.type === "MARKDOWN") {
                            navigate(
                                `/app/areas/${studyAreaId}/materiais/${created._id}?edit=1`,
                            );
                        }
                    }}
