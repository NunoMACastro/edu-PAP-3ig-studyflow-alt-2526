...
export function StudyAreaMaterialsPage({ studyAreaId }: StudyAreaMaterialsPageProps) {
    ...
    return (
        <section className="space-y-6">
            <PageHeader
                action={<><button aria-expanded={createOpen} className="sf-button-primary" onClick={() => setCreateOpen(true)} type="button">Novo material</button><button aria-expanded={importOpen} className="sf-button-secondary" onClick={() => setImportOpen(true)} type="button">Importar link</button></>}
                title="Materiais"
