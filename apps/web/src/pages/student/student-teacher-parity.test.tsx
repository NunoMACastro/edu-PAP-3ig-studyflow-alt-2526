...
describe("paridade professor → aluno", () => {
    ...
    it("mostra descrição e entrada para materiais oficiais da disciplina", async () => {
        ...
        expect(screen.getByRole("button", { name: "Ativas" }).getAttribute("aria-pressed")).toBe("true");
        expect(screen.getByRole("link", { name: "Abrir disciplina" }).getAttribute("href")).toBe("/app/disciplinas/subject-active");
    });
    ...
    it("não mostra ações IA sem endpoint histórico para disciplinas arquivadas", async () => {
        ...
        expect(screen.getByRole("link", { name: "Consultar disciplina" })).toBeTruthy();
    });
    ...
    it("apresenta revisão ativa e disponibilidade para IA no catálogo seguro", async () => {
        ...
        expect(screen.getByText("Disponível para IA").className).toContain("whitespace-nowrap");
        await waitFor(() => expect(api.listStudentOfficialMaterials).toHaveBeenCalledWith("subject-active", { limit: 24 }));
    });

    it("alinha as ações compactas dos cards e mantém nomes e tooltips acessíveis", async () => {
        api.listStudentOfficialMaterials.mockResolvedValueOnce({
            items: [{
                _id: "pdf-id",
        ...
        expect(await screen.findByText("Resumo PDF")).toBeTruthy();
        const actions = screen.getByRole("group", { name: "Ações de Resumo PDF" });
        expect(actions.className).toContain("mt-auto");
        expect(actions.className).toContain("justify-end");
        expect(within(actions).getByRole("link", { name: "Consultar material" })).toBeTruthy();
        expect(within(actions).getByRole("link", { name: "Abrir PDF" })).toBeTruthy();
        expect(within(actions).getByRole("link", { name: "Descarregar" })).toBeTruthy();
        expect(within(actions).getByText("Consultar material").getAttribute("data-tooltip-side")).toBe("top");
        expect(within(actions).getByText("Abrir PDF").getAttribute("data-tooltip-side")).toBe("top");
        expect(within(actions).getByText("Descarregar").getAttribute("data-tooltip-side")).toBe("top");
    });
});
