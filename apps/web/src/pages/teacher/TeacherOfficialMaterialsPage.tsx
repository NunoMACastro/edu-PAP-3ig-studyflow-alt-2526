...
type MaterialFormType = MaterialType | "MARKDOWN_FILE";
...
function statusLabel(
    ...
    if (material.status === "DRAFT") return "Rascunho";
...
export function TeacherOfficialMaterialsPage({ subjectId }: { subjectId: string }) {
    ...
    const [type, setType] = useState<MaterialFormType>("TEXT");
    ...
    useEffect(() => {
        if (searchParams.get("tipo") === "markdown") {
            setType("MARKDOWN");
        }
    }, [searchParams]);
    ...
    async function handleSubmit(event: FormEvent): Promise<void> {
        ...
        setIsSaving(true);
        try {
            let created: OfficialMaterial;
            if (type === "PDF" || type === "DOCX" || type === "MARKDOWN_FILE") {
                if (!file) throw new Error("Seleciona um ficheiro compatível.");
                created = await uploadOfficialMaterialFile(subjectId, { title, file });
            } else {
                created = await createOfficialMaterial(subjectId, {
                    title,
                    type,
                    textContent: type === "TEXT" ? textContent : undefined,
                    sourceUrl: type === "URL" ? sourceUrl : undefined,
                    markdownSource: type === "MARKDOWN" ? textContent : undefined,
                });
            }
            resetForm();
            await refresh();
            ...
            if (created.type === "MARKDOWN") {
                ...
                const detailParams = new URLSearchParams({ edit: "1" });
                if (returnTo) detailParams.set("returnTo", returnTo);
                navigate(
                    `/app/professor/disciplinas/${subjectId}/materiais/${created._id}?${detailParams.toString()}`,
                );
            }
        } catch (caught) {
    ...
    return (
        <section className="space-y-6">
                        ...
                        return (
                            <article className="sf-list-card flex min-w-0 flex-col gap-3" key={material._id}>
                                ...
                                <div className="mt-auto flex flex-wrap gap-2 pt-2">
                                    {material.type !== "MARKDOWN" && (!material.availableToAi || !job) ? (
                                    ...
                                    {material.type === "MARKDOWN" ? <Link className="sf-button-secondary" to={`/app/professor/disciplinas/${subjectId}/materiais/${material._id}`}>Abrir</Link> : null}
            ...
            <SidePanel closeDisabled={isSaving} description="Adiciona texto, URL, PDF, DOCX ou Markdown à disciplina." onClose={() => setCreateOpen(false)} open={createOpen} size={type === "MARKDOWN" ? "wide" : "default"} title="Criar material oficial">
                <form className="space-y-4" id="criar-material" onSubmit={(event) => void handleSubmit(event)} ref={createFormRef}>
                    {actionError ? <InlineNotice tone="danger">{actionError}</InlineNotice> : null}
                    <label className="block space-y-2" htmlFor="official-material-type"><span>Tipo de material</span><select id="official-material-type" value={type} onChange={(event) => { setType(event.target.value as MaterialFormType); setFile(null); }}><option value="TEXT">Texto processado</option><option value="URL">Referência URL</option><option value="MARKDOWN">Markdown no editor</option><option value="MARKDOWN_FILE">Upload Markdown</option><option value="PDF">PDF</option><option value="DOCX">DOCX</option></select></label>
                    ...
                    {type === "MARKDOWN" ? <MarkdownEditor error={actionError} isDirty={textContent.length > 0} isSaving={isSaving} onChange={setTextContent} onSave={() => createFormRef.current?.requestSubmit()} saveLabel="Guardar rascunho" value={textContent} /> : null}
                    ...
                    {type === "MARKDOWN_FILE" ? <label className="block space-y-2" htmlFor="official-markdown-file"><span>Ficheiro MD</span><input accept=".md,text/markdown,text/plain" id="official-markdown-file" required type="file" onChange={(event) => setFile(event.target.files?.[0] ?? null)} /><span className="block text-sm text-studyflow-text/65">Máximo de 128 KiB. O upload cria um rascunho editável.</span></label> : null}
                    {type !== "MARKDOWN" ? <button className="sf-button-primary" disabled={isSaving}>{isSaving ? "A enviar..." : "Guardar material"}</button> : null}
                </form>
            </SidePanel>
...
/** Aceita apenas regressos internos da área docente. */
