...
export function TeacherGuidedStudyRoomDetailPage({ classId, roomId }: { classId: string; roomId: string }) {
    ...
    return (
        <section className="space-y-6">
            ...
            <section className="sf-surface space-y-4"><SectionHeader title="Atividade" description="Instruções e recursos disponibilizados aos alunos." /><p className="whitespace-pre-wrap text-sm leading-6">{room.description}</p>{room.startsAt ? <p className="text-sm">Início previsto: {new Date(room.startsAt).toLocaleString("pt-PT")}{room.durationMinutes ? ` · ${room.durationMinutes} minutos` : ""}</p> : null}<div className="grid gap-2">{room.materials.map((material) => <article className="sf-surface-subtle space-y-2" key={material._id}><strong>{material.title}</strong><p className="text-sm text-studyflow-text/70">{material.status === "PROCESSED" ? "Material processado" : "Referência"}</p>{material.type === "MARKDOWN" && material.markdownSource ? <MarkdownViewer className="text-sm" source={material.markdownSource} /> : null}<div className="flex flex-wrap gap-2"><OfficialMaterialFileActions material={material} />{material.type === "MARKDOWN" ? <a className="sf-button-secondary" href={`/app/professor/disciplinas/${material.subjectId}/materiais/${material._id}`}>Abrir e editar</a> : null}</div></article>)}</div>{room.invalidMaterialIds.length ? <InlineNotice tone="attention">Existem {room.invalidMaterialIds.length} referências de materiais legadas inválidas. Edita a sala para as remover.</InlineNotice> : null}{room.officialTest ? <a className="sf-button-secondary" href={`/app/professor/disciplinas/${room.officialTest.subjectId}/testes`}>Abrir mini-teste: {room.officialTest.title}</a> : null}</section>
            <section className="sf-surface space-y-4"><SectionHeader title="Participação" description="Estados factuais; visualizar não equivale a presença." /><Toolbar ariaLabel="Filtrar participação"><label>Estado<select onChange={(event) => setStatusFilter(event.target.value)} value={statusFilter}><option value="ALL">Todos</option><option value="NOT_VIEWED">Não visualizaram</option><option value="VIEWED">Visualizaram</option><option value="COMPLETED">Concluíram</option></select></label></Toolbar><div className="divide-y divide-studyflow-border/10">{visibleStudents.map((student) => <div className="flex flex-wrap items-center justify-between gap-2 py-3" key={student.studentId}><div className="min-w-0"><p className="truncate font-semibold">{student.displayName}</p><p className="truncate text-xs text-studyflow-text/60">{student.email}</p></div><StatusBadge tone={student.status === "COMPLETED" ? "brand" : "neutral"}>{student.status === "NOT_VIEWED" ? "Não visualizou" : student.status === "VIEWED" ? "Visualizou" : "Concluiu"}</StatusBadge></div>)}</div></section>
            <section className="sf-surface space-y-4"><SectionHeader title="Conversas IA supervisionadas" description="Consulta read-only. O conteúdo não é partilhado com os restantes alunos." />{progress.students.length ? <label className="block max-w-md">Aluno<select onChange={(event) => setStudentFilter(event.target.value)} value={studentFilter}><option value="">Todos</option>{progress.students.map((student) => <option key={student.studentId} value={student.studentId}>{student.displayName} — {student.email}</option>)}</select></label> : null}{history.items.length === 0 ? <EmptyState title="Ainda não existem conversas IA" /> : <div className="space-y-3">{history.items.map((item) => <article className="sf-surface-subtle space-y-2" key={item._id}><p className="text-xs uppercase text-studyflow-brandText">{item.studentDisplayName} · {item.studentEmail} · {item.createdAt ? new Date(item.createdAt).toLocaleString("pt-PT") : ""}</p><p><strong>Pergunta:</strong> {item.question}</p><p className="whitespace-pre-wrap"><strong>Resposta:</strong> {item.answer}</p><p className="text-sm text-studyflow-text/70">Fontes: {item.sources.map((source) => source.title).join(", ")}</p></article>)}</div>}{history.nextCursor ? <button className="sf-button-secondary" onClick={() => void loadMore()} type="button">Carregar mais</button> : null}{room.subjectId ? <a className="sf-button-secondary" href={`/app/professor/disciplinas/${room.subjectId}/chat`}>Abrir chat da disciplina</a> : null}</section>
        </section>
    );
}
