...
/** Conserva o painel atual para o professor publicar Markdown e regressar ao seletor. */
function markdownCreationHref(
    subjectId: string,
    classId: string,
    editingRoomId?: string,
): string {
    const returnTo = editingRoomId
        ? `/app/professor/turmas/${classId}/salas-guiadas?edit=${editingRoomId}`
        : `/app/professor/turmas/${classId}/salas-guiadas?create=1`;
    const query = new URLSearchParams({ tipo: "markdown", returnTo });
    return `/app/professor/disciplinas/${subjectId}/materiais?${query.toString()}#criar-material`;
}
