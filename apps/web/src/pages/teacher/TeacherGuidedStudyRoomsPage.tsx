...
const ROOM_STATUS_LABELS: Record<GuidedStudyRoom["status"], string> = {
    OPEN: "Aberta",
    CLOSED: "Encerrada",
};

/**
 * Traduz o estado técnico de uma sala guiada para a interface docente.
 *
 * @param status Estado devolvido pela API.
 * @returns Estado legível em português.
 */
function getRoomStatusLabel(status: GuidedStudyRoom["status"]): string {
...
export function TeacherGuidedStudyRoomsPage({ classId }: { classId: string }) {
    ...
    return (
        <section className="grid gap-6 lg:grid-cols-[380px_1fr]">
            <form className="sf-panel space-y-4" id="criar-sala-guiada" onSubmit={(event) => void handleSubmit(event)}>
            ...
            <div className="grid gap-3">
                ...
                {rooms.map((room) => (
                    <article className="sf-panel" key={room._id}>
                        ...
                        <p className="mt-2 text-xs uppercase text-studyflow-text">{getRoomStatusLabel(room.status)}</p>
                    </article>
                ))}
            </div>
        </section>
