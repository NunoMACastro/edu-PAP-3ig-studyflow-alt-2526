...
export function TeacherAiContentReviewsPage({ subjectId }: { subjectId: string }) {
    ...
    return (
        <section className="space-y-6">
            ...
            {!loading && !loadError ? (
                <>
                    <Toolbar ariaLabel="Filtros dos conteúdos docentes">
                    ...
                    {visibleReviews.length === 0 ? (
                        <EmptyState
                            ...
                            description={reviews.length === 0
                                ? "Cria conteúdo docente a partir de um material oficial processado."
            ...
            <SidePanel
                ...
                description={selectedReview ? `Material oficial: ${selectedReview.materialTitle}` : "Detalhe do conteúdo docente."}
                onClose={() => setSelectedReview(null)}
                open={Boolean(selectedReview)}
                title={selectedReview ? `${TYPE_LABELS[selectedReview.contentType]} para aprovação` : "Conteúdo docente"}
            >
                {selectedReview ? (
