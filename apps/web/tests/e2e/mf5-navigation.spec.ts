...
test("MF5 navegação: links mudam por role e página atual fica marcada", async ({
    ...
    await expect(overviewPanel.getByRole("link", { name: "Abrir sinais a rever" })).toBeVisible();
    await expect(
    ...
    await expect(demoClassCard.getByRole("link", { name: "Disciplinas de 12.º Demo" })).toBeVisible();
    ...
    await expect(
        alphaClassCard.getByRole("button", { name: "Adicionar primeiro aluno a 11.º Alpha" }),
    ).toBeVisible();
    ...
    await expect(
        mathSubjectCard.getByRole("link", { name: "Contextos de Matemática" }),
    ).toBeVisible();
    ...
    await sortedAlphaClassCard
        .getByRole("button", { name: "Adicionar primeiro aluno a 11.º Alpha" })
        .click();
    await expect(
...
test("MF5 navegação: acompanhamento docente fica acessível", async ({ page }) => {
    ...
    const teacherNav = page.getByRole("navigation", { name: "Navegação principal" });
    await teacherNav.getByRole("link", { name: "Acompanhamento" }).click();
    await expect(page).toHaveURL(/\/app\/professor\/acompanhamento$/);
    ...
    await expect(teacherNav.getByRole("link", { name: "Acompanhamento" })).toHaveAttribute(
        "aria-current",
        "page",
    );
    await expect(teacherNav.getByRole("link", { name: "Estudo" })).toHaveCount(0);
