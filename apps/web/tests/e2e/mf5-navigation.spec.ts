...
test("MF5 navegação: links mudam por role e página atual fica marcada", async ({
    ...
    await expect(studentNav.getByRole("link", { name: "Estudar" })).toHaveAttribute(
        "aria-current",
        "page",
    );
    ...
    await expect(
        page.getByRole("link", { name: "Conteúdos aprovados de Matemática" }),
    ).toBeVisible();
    ...
    await expect(
        mathSubjectCard.getByRole("link", { name: "Conteúdos aprovados de Matemática" }),
    ).toBeVisible();
    await expect(
