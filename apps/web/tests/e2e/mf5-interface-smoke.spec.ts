...
test("MF5 interface: aluno e professor veem paginas claras com uma acao principal", async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: "Organizar rotinas" })).toBeVisible();
    ...
    const classForm = page.locator("form").filter({ hasText: "Criar turma" });
    ...
    await page.getByPlaceholder("Descrição").fill("Disciplina criada pelo smoke MF5.");
