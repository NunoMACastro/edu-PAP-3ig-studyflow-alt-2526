...
test("MF2 smoke: professor e aluno percorrem projectos, testes, indexacao e IA privada", async ({ page }) => {
    ...
    await page.getByLabel("ID do material oficial").fill(officialMaterial!._id);
    await page.getByLabel("Conteúdo gerado a rever").fill("Resumo IA validado pelo professor.");
    ...
    await expect(
        reviewCard.getByText(/^Aprovado · material [a-f0-9]{24}$/),
    ).toBeVisible();
    ...
    await page
        .getByLabel("Dificuldades", { exact: true })
    ...
    await expect(page.getByRole("listitem").filter({ hasText: /enunciado/i }).first()).toBeVisible();
    ...
    await loginAs(page, teacher);
    await page.goto(`/app/professor/disciplinas/${subjectId}/testes`);
    const completedTestCard = page.locator("article").filter({
        hasText: officialTestTitle,
    });
