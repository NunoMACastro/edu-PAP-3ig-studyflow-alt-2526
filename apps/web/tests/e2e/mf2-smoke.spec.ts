...
test("MF2 smoke: professor e aluno percorrem projectos, testes, indexacao e IA privada", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "Revisões IA", exact: true })).toBeVisible();
    await page.getByRole("button", { name: "Nova revisão" }).click();
    const reviewDialog = page.getByRole("dialog", { name: "Criar revisão IA" });
    ...
    await page.getByRole("dialog", { name: "Quiz para revisão" }).getByRole("button", { name: "Aprovar" }).click();
    ...
    await expect(studentPdfCard.getByRole("link", { name: "Abrir PDF" })).toBeVisible();
    ...
    await expect(page.getByRole("heading", { name: "Conteúdos IA aprovados" })).toBeVisible();
    ...
    const projectPlanLink = studentProjectCard.getByRole("link", { name: "Criar plano IA" });
    ...
    await expect(page.getByRole("heading", { name: "Plano IA do projeto" })).toBeVisible();
    ...
    await expect(page.getByRole("heading", { name: "Áreas de estudo", exact: true })).toBeVisible();
    ...
    const approvedReviewDialog = page.getByRole("dialog", { name: "Quiz para revisão" });
