 ...
 *
 * @param page Página Playwright autenticada.
...
async function apiGet<T>(page: Page, path: string): Promise<T> {
    return page.evaluate(async (apiPath) => {
        ...
        if (!response.ok) {
            throw new Error(`GET ${apiPath} devolveu HTTP ${response.status}.`);
        }
...
test("MF2 smoke: professor e aluno percorrem projectos, testes, indexacao e IA privada", async ({ page }) => {
    ...
    const classForm = page.locator("form").filter({ hasText: "Turmas" });
    ...
    await classCard.getByPlaceholder("email do aluno").fill(student.email);
    ...
    await expect(page.locator("article").filter({ hasText: guidedRoomTitle })).toBeVisible();
    ...
    await page.getByLabel("Título").fill(privateMaterialTitle);
    await page
        .getByLabel("Texto")
