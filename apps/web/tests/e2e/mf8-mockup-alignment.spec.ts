...
/**
 * Lê credenciais E2E sem escrever segredos no teste.
 *
 ...
 * @returns Promise resolvida quando a shell autenticada fica visível.
 */
...
test("MF8 RNF38 mostra checklist de alinhamento ao mockup com rotas reais", async ({
    ...
    await expect(page.getByRole("link", { name: "/app/salas", exact: true })).toBeVisible();
    await expect(
        page.getByRole("link", { name: "/app/professor/turmas", exact: true }),
    ).toBeVisible();
