...
test("MF1 smoke: professor e aluno percorrem os fluxos principais com sessao real", async ({ page }) => {
    ...
    await page.getByRole("button", { name: "Perguntar" }).click();
    const currentRoomAnswer = page.locator("article").filter({
        has: page.getByRole("heading", { name: "Resposta" }),
    });
    await expect(currentRoomAnswer).toBeVisible();
    await expect(
        currentRoomAnswer.getByText("Resposta deterministica da IA da sala"),
    ).toBeVisible();
    await expect(currentRoomAnswer.getByText(shareTitle)).toBeVisible();
});
