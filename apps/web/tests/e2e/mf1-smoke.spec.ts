...
test("MF1 smoke: professor e aluno percorrem os fluxos principais com sessao real", async ({ page }) => {
    ...
    await page
        ...
        .getByRole("link", { name: "Estudar" })
        .click();
    const studentClassCard = page.locator("article").filter({ hasText: className });
    await expect(studentClassCard).toBeVisible();
    await studentClassCard.getByRole("link", { name: "Abrir turma" }).click();

    const studentSubjectCard = page.locator("article").filter({ hasText: subjectName });
    await expect(studentSubjectCard).toBeVisible();
    await studentSubjectCard.getByRole("link", { name: "Abrir disciplina" }).click();
    await page.getByRole("button", { name: "Abrir Assistente de estudo" }).click();
    const subjectAssistant = page.getByRole("dialog", { name: "Assistente de estudo" });
    await subjectAssistant.getByRole("button", { name: "Começar nova conversa" }).click();
    const classQuestion = "Explica funcao afim com base no material oficial.";
    await subjectAssistant.getByLabel("Pergunta ao Assistente").fill(classQuestion);
    await subjectAssistant.getByRole("button", { name: "Enviar" }).click();
    await expect(
        subjectAssistant.getByLabel("Mensagens da conversa").getByText(classQuestion, {
            exact: true,
        }),
    ).toBeVisible();
    await expect(subjectAssistant.getByText("A preparar resposta...")).toHaveCount(0);
    ...
    await page
        .getByRole("navigation", { name: "Navegação principal" })
        .getByRole("link", { name: "Em grupo", exact: true })
        .click();
    await expect(page.getByRole("heading", { name: "Em grupo" })).toBeVisible();
    await page.getByRole("button", { name: "Nova sala" }).click();
    const roomForm = page.getByRole("dialog", { name: "Nova sala partilhada" }).locator("form");
    ...
    const sharesHref = await roomCard.getByRole("link", { name: "Abrir sala" }).getAttribute("href");
    const roomId = extractIdFromHref(sharesHref, /\/salas\/([^/]+)$/);
    await roomCard.getByRole("link", { name: "Abrir sala" }).click();

    await expect(page.getByRole("heading", { name: "Partilhas da sala" })).toBeVisible();
    await page.getByRole("button", { name: "Nova partilha" }).click();
    ...
    await expect(page).toHaveURL(new RegExp(`/app/assistente/novo/STUDY_ROOM/${roomId}$`));
    await page.getByRole("button", { name: "Começar conversa" }).click();
    await expect(page.getByRole("button", { name: "Aceitar e ativar IA" })).toBeVisible();
    await grantAiConsent(page, "ROOM_AI");
    await page.reload();
    await page.getByLabel("Pergunta ao Assistente").fill("O que diz o apontamento sobre taxa de variacao?");
