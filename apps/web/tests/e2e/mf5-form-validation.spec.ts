...
async function createStudyArea(page: Page): Promise<string> {
    ...
    const areaLink = page.locator("article").filter({ hasText: areaName }).getByRole("link", { name: "Continuar" });
...
/**
 * Cria uma fixture estável para a página de turmas no teste de validação de formulário.
 *
 * @returns Lista mínima compatível com `/api/teacher/classes`.
 */
function teacherClassesFixture() {
    return [
        {
...
test("MF5 valida submissão de material antes de chamar a API", async ({ page }) => {
    ...
    const materialForm = page.getByRole("dialog", { name: "Novo material" }).locator("form");
