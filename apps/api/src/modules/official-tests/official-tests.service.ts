...
export class OfficialTestsService {
    ...
    async listPublishedForStudent(
        ...
        const views = await Promise.all(
            tests.map(async (test) => {
                ...
                return this.toStudentView(test, attemptsUsed);
            }),
        );
        return views;
    }
    ...
    async changeStatus(
        ...
        if (changed) return this.toView(changed);
    ...
    async listMyAttempts(
        ...
        return attempts.map((attempt) =>
            this.toAttemptView(attempt, solutionUnlocked),
        );
    }
    ...
    private toStudentView(test: {
        ...
        questions: OfficialTestQuestion[];
        createdAt?: Date;
    }, attemptsUsed: number): OfficialTestStudentView {
    ...
    private toAttemptView(attempt: {
        ...
        answeredAt: Date;
    }, solutionUnlocked: boolean): OfficialTestAttemptView {
        return {
            ...
            attemptsRemaining: Math.max(
                0,
                MAX_OFFICIAL_TEST_ATTEMPTS - attempt.attemptNumber,
            ),
