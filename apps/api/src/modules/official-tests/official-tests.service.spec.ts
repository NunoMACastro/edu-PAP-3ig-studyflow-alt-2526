...
describe("OfficialTestsService", () => {
    ...
    it("submete tentativa, calcula pontuação e persiste studentId da sessão", async () => {
        const { attemptModel, classLearningActivityService, service } = makeService();
        ...
        expect(classLearningActivityService.recordBestEffort).toHaveBeenCalledWith({
            ...
            sourceEventKey: "official-test-attempt:507f1f77bcf86cd799439017",
    ...
    it("bloqueia o fecho enquanto o teste for obrigatório numa sala guiada aberta", async () => {
        const { guidedRoomModel, notificationsService, service, testModel } =
            makeService();
        guidedRoomModel.exists.mockResolvedValueOnce(true);
        ...
        expect(testModel.findOneAndUpdate).not.toHaveBeenCalled();
    ...
    it("edita conteúdo enquanto o teste permanece DRAFT", async () => {
        ...
        expect(testModel.findOneAndUpdate).toHaveBeenCalledWith(
            ...
            { new: true, runValidators: true, session: { id: "session" } },
        );
    });
