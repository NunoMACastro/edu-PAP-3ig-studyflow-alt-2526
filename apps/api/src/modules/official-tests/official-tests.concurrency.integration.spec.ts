...
describe("OfficialTestsService — invariantes concorrentes", () => {
    ...
    beforeAll(async () => {
        ...
        service = new OfficialTestsService(
            ...
            { enqueueClassEvent: jest.fn().mockResolvedValue({ state: "PENDING" }) } as never,
            connection,
        );
    ...
    it("nunca confirma uma sala aberta ligada a um teste fechado", async () => {
        const test = await createPublishedTest();
        const roomId = new Types.ObjectId();

        const [roomCreation, closing] = await Promise.allSettled([
            connection.transaction(async (session) => {
                await service.reserveGuidedRoomDependency(
                    teacher.id,
                    classId,
                    String(test._id),
                    session,
                    ...
                    [
                        {
                            ...
                            title: "Sala concorrente",
                            description: "Sala que exige o mini-teste publicado.",
                            materialIds: [],
        ...
        expect([roomCreation, closing].filter((result) => result.status === "fulfilled"))
            .toHaveLength(1);
        const [persistedTest, persistedRoom] = await Promise.all([
            testModel.findById(test._id).lean(),
            guidedRoomModel.findById(roomId).lean(),
        ]);
        expect(
            persistedTest?.status === "CLOSED" && persistedRoom?.status === "OPEN",
        ).toBe(false);
        if (persistedRoom?.status === "OPEN") {
            expect(persistedTest?.status).toBe("PUBLISHED");
        } else {
            expect(persistedTest?.status).toBe("CLOSED");
        }
    });

    it("lineariza o PATCH que associa um teste contra o fecho concorrente", async () => {
        const test = await createPublishedTest();
        const room = await guidedRoomModel.create({
            classId: new Types.ObjectId(classId),
            subjectId: new Types.ObjectId(subjectId),
            teacherId: new Types.ObjectId(teacher.id),
            title: "Sala editável",
            description: "Sala ainda sem mini-teste associado.",
            materialIds: [],
            aiEnabled: false,
            status: "OPEN",
        });
        let releaseValidation!: () => void;
        let signalValidationReached!: () => void;
        const validationReached = new Promise<void>((resolve) => {
            signalValidationReached = resolve;
        });
        const validationCanReturn = new Promise<void>((resolve) => {
            releaseValidation = resolve;
        });
        const originalFindPublished = service.findOwnedPublishedTest.bind(service);
        const validationSpy = jest
            .spyOn(service, "findOwnedPublishedTest")
            .mockImplementationOnce(async (teacherId, testId) => {
                const view = await originalFindPublished(teacherId, testId);
                signalValidationReached();
                await validationCanReturn;
                return view;
            });

        try {
            const roomUpdate = guidedRoomsService.update(
                teacher,
                classId,
                String(room._id),
                { officialTestId: String(test._id) },
            );
            ...
            await expect(roomUpdate).rejects.toMatchObject({
                response: expect.objectContaining({
            ...
            const [persistedTest, persistedRoom] = await Promise.all([
                testModel.findById(test._id).lean(),
                guidedRoomModel.findById(room._id).lean(),
            ]);
            expect(persistedTest?.status).toBe("CLOSED");
            expect(persistedRoom?.status).toBe("OPEN");
            expect(persistedRoom?.officialTestId).toBeUndefined();
        } finally {
            releaseValidation();
