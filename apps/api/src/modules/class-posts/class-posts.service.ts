...
export type StudentClassPostView = Omit<ClassPostView, "teacherId">;
...
export class ClassPostsService {
    ...
    async createPost(
        ...
        return this.runInTransaction(async (session) => {
            ...
            const post = session
                ? (await this.postModel.create([document], { session }))[0]
                : await this.postModel.create(document);
            const view = this.toPostView(post.toObject());
            const notification = {
                classId: schoolClass._id,
                idempotencyKey: `class-post:${view._id}:published`,
                type: "CLASS_POST_PUBLISHED",
                title: view.title ?? "Nova publicação da turma",
                body: "O professor publicou um novo conteúdo na turma.",
                targetPath: `/app/turmas/${schoolClass._id}/publicacoes`,
            } as const;
            if (session) {
    ...
    async listStudentPosts(
        actor: AuthenticatedUser,
        classId: string,
    ): Promise<StudentClassPostView[]> {
        this.assertStudent(actor);
        const schoolClass = await this.classesService.ensureStudentHistoricalEnrollment(
            actor.id,
            classId,
        );
        const posts = await this.listByClass(schoolClass._id);
        return posts.map(({ teacherId: _teacherId, ...post }) => post);
    }
    ...
    /** Mantém a publicação e o respetivo evento na mesma unidade de commit. */
