...
export class StudyGroupAiService {
    ...
    async ask(
        ...
        const group = await this.studyGroupsService.ensureMember(actor.id, groupId);
        ...
        try {
            const execution = await this.aiExecution.executeAuthorized(
                authorization,
                {
                    ...
                    buildPrompt: (limitedSources, conversationHistory) =>
                        this.buildPrompt(
                            input.question,
                            [...limitedSources],
                            conversationHistory,
            ...
            const document = await this.answerModel.create({
                ...
                ...(executionContext
                    ...
                    : {}),
                citationSnapshots: selectedSources.map((source) => ({
                    label: source.title,
                    kind: "GROUP_RESOURCE",
                })),
            });
