...
describe("StudentAiContextResolverService", () => {
    ...
    it("oculta grupos da descoberta mas mantém a resolução direta compatível", async () => {
        const fixture = makeFixture();
        const groupId = new Types.ObjectId().toString();
        fixture.groups.listMyGroups.mockResolvedValue([{ _id: groupId, title: "Grupo antigo" }]);

        await expect(fixture.service.list(student, { limit: 20 })).resolves.toMatchObject({
            items: [],
        });
        expect(fixture.groups.listMyGroups).not.toHaveBeenCalled();
    });
});

/** Cria dependências estritamente controladas para testar só a resolução segura. */
function makeFixture() {
    const classes = { listStudentClasses: jest.fn().mockResolvedValue([]) };
    const subjects = {
    ...
    const guided = {
        ...
        listAllForStudent: jest.fn().mockResolvedValue({ items: [], nextCursor: null }),
    };
