...
describe("ClassAiService", () => {
    ...
    it("mantém consentimento e policy entre perfil e materiais oficiais", () => {
        ...
        const consentIndex = source.indexOf(
            'this.aiConsentsService.assertGranted(actor.id, "CLASS_AI")',
            profileIndex,
        );
        const policyIndex = source.indexOf(
            'this.aiModelPoliciesService.resolveForUse("CLASS_AI")',
            consentIndex,
        );
        const materialsIndex = source.indexOf("listProcessedForSubject", policyIndex);
        const promptLimitIndex = source.indexOf(
            "assertPromptWithinLimit(prompt, policy)",
            materialsIndex,
        );
        const quotaIndex = source.indexOf(
            "this.aiQuotasService.reserveUsage",
            promptLimitIndex,
        );
        const providerIndex = source.indexOf(
            "this.aiProvider.generateClassAnswer",
            quotaIndex,
        );

        expect(membershipIndex).toBeGreaterThanOrEqual(0);
        expect(profileIndex).toBeGreaterThan(membershipIndex);
        expect(consentIndex).toBeGreaterThan(profileIndex);
        expect(policyIndex).toBeGreaterThan(consentIndex);
        expect(materialsIndex).toBeGreaterThan(policyIndex);
        expect(promptLimitIndex).toBeGreaterThan(materialsIndex);
