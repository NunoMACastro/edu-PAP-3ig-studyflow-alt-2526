...
describe("StudyToolsService", () => {
    ...
    it("aceita apenas o hash interno do Assistente e reutiliza o artefacto", async () => {
        const { artifactModel, aiProvider, service } = makeService();
        artifactModel.findOne.mockReturnValueOnce({
            lean: jest.fn().mockResolvedValue({
                ...makeQuizArtifact(),
                type: "FLASHCARDS",
                contentJson: { title: "Cards", cards: [] },
            }),
        });
        const generationKey = `assistant-artifact:${"a".repeat(64)}`;

        await expect(
            service.generateStudyToolForAssistant(
                userId,
                studyAreaId,
                { type: "FLASHCARDS" },
                {
        ...
        ).resolves.toMatchObject({ type: "FLASHCARDS" });
        expect(aiProvider.generateStudyTool).not.toHaveBeenCalled();
    });
