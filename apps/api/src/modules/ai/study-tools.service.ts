...
export class StudyToolsService {
    ...
    /** Reutiliza o pipeline governado e associa o resultado ao Assistente. */
    generateStudyToolForAssistant(
        userId: string,
        studyAreaId: string,
        input: CreateStudyToolDto,
        options: { conversationId: string; generationKey: string },
    ) {
        return this.generateStudyTool(
            userId,
            studyAreaId,
            input,
    ...
    /**
     * Reutiliza o pipeline governado com uma entrada congelada pelo Assistente,
     * sem voltar a ler a área, a conversa ou qualquer fonte mutável.
     */
    async generateStudyToolFromAssistantSnapshot(
        ...
        try {
            const execution = await this.aiExecution.execute({
                ...
                buildPrompt: (sources, conversationHistory) =>
                    buildStudyToolPrompt({
                        areaName: snapshot.contextLabel,
                        type,
                        topic: input.topic,
    ...
    async submitQuizAttempt(
        ...
        return this.recordQuizAttempt(
            userId,
            artifact as OwnedQuizArtifact,
            input,
        );
    }

    /**
     * Regista uma tentativa de um material transversal já autorizado por owner.
     * A camada do arquivo revalida primeiro se o destino continua ativo.
     */
    async submitQuizAttemptForOwnedArtifact(
        userId: string,
        artifactId: string,
        input: CreateQuizAttemptDto,
    ) {
        if (!Types.ObjectId.isValid(artifactId)) throw this.artifactNotFound();
        const artifact = await this.artifactModel
            .findOne({
    ...
    /** Valida o quiz, calcula o resultado e persiste apenas dados do owner. */
    private async recordQuizAttempt(
        userId: string,
        artifact: OwnedQuizArtifact,
        input: CreateQuizAttemptDto,
    ) {
        ...
        const attempt = await this.quizAttemptModel.create({
            userId: new Types.ObjectId(userId),
            ...(artifact.studyAreaId ? { studyAreaId: artifact.studyAreaId } : {}),
            targetKind:
                artifact.targetKind ??
                (artifact.studyAreaId ? "STUDY_AREA" : undefined),
            targetId: artifact.targetId ?? artifact.studyAreaId,
