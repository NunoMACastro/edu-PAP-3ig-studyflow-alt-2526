...
export interface AiProvider {
    ...
    generateAdaptiveExplanation(input: {
        prompt: string;
    }): Promise<AdaptiveExplanationResult>;
    generateRoomAnswer(input: { prompt: string }): Promise<RoomAiResult>;
    generateClassAnswer(input: { prompt: string }): Promise<ClassAiResult>;
    generateProjectPlan(input: { prompt: string }): Promise<ProjectAiPlanResult>;
    generatePrivateAreaAnswer(input: {
        prompt: string;
...
export class OpenAiProvider implements AiProvider {
     ...
     */
    async generateAdaptiveExplanation(input: {
        prompt: string;
    }): Promise<AdaptiveExplanationResult> {
    ...
    async generateRoomAnswer(input: { prompt: string }): Promise<RoomAiResult> {
        return this.createJsonResponse<RoomAiResult>(input.prompt);
    }
    ...
    async generateClassAnswer(input: { prompt: string }): Promise<ClassAiResult> {
        return this.createJsonResponse<ClassAiResult>(input.prompt);
    }
    ...
    async generateProjectPlan(input: {
        ...
        return this.createJsonResponse<ProjectAiPlanResult>(input.prompt);
    }
    ...
    async generatePrivateAreaAnswer(input: {
        ...
        return this.createJsonResponse<PrivateAreaAiResult>(input.prompt);
    }
...
export class E2eFakeAiProvider implements AiProvider {
    ...
    async generateSummary(input: { prompt: string }): Promise<SummaryResult> {
        return {
            ...
            sourceMaterialIds: this.extractIds(input.prompt, /^Material \d+ \(([^)]+)\)/gm),
        };
    }
     ...
     * @returns Resposta validada, limitada às fontes e pronta para persistência ou apresentação.
     */
    async generateStudyTool(): Promise<Record<string, unknown>> {
        return {
            title: "Ferramenta E2E",
            content: "Conteudo deterministico usado pelo smoke E2E.",
        };
    }
     ...
     *
     * @param prompt prompt necessário para executar extract first id sem depender de estado global.
     * @param pattern pattern necessário para executar extract first id sem depender de estado global.
     ...
     *
     * @param prompt prompt necessário para executar extract ids sem depender de estado global.
     * @param pattern pattern necessário para executar extract ids sem depender de estado global.
     * @returns Valor de artefactos de IA no contrato esperado pelo chamador.
     */
