...
import { isE2eRuntimeDoubleEnabled } from "../../../common/config/e2e-runtime-boundary.js";
...
export class OpenAiProvider implements AiProvider {
        ...
        try {
            ...
            if (status === 401 || status === 403) {
                throw new ServiceUnavailableException({
                    ...
                    message: "A configuração do serviço de IA não foi aceite.",
                });
            }
            if (status === 404) {
                throw new ServiceUnavailableException({
                    code: "AI_PROVIDER_MODEL_UNAVAILABLE",
                    message: "O modelo de IA configurado não está disponível.",
                });
            }
            if (status === 429) {
                throw new ServiceUnavailableException({
                    code: "AI_PROVIDER_RATE_LIMITED",
                    message: "O serviço de IA está temporariamente com demasiados pedidos.",
                });
            }
    ...
    /**
     * Extrai apenas o status HTTP seguro do SDK, sem propagar mensagens,
     * headers, request IDs ou detalhes potencialmente sensíveis do provider.
     */
 ...
 *
 * A ativação exige `STUDYFLOW_E2E_MODE=true`, `NODE_ENV=test` e a flag
...
export function createAiProvider(): AiProvider {
    if (isE2eRuntimeDoubleEnabled("STUDYFLOW_E2E_FAKE_AI")) {
        return new E2eFakeAiProvider();
    }
    if (isDemoFakeAiEnabled()) {
        return new E2eFakeAiProvider();
    }
...
/**
 * Mantém o provider determinístico fora de produção e exige duas guardas para
 * que uma variável isolada nunca desative chamadas reais por engano.
 */
function isDemoFakeAiEnabled(): boolean {
    const demoMode = process.env.STUDYFLOW_DEMO_MODE === "true";
    const fakeAi = process.env.STUDYFLOW_DEMO_FAKE_AI === "true";
    if (!demoMode && !fakeAi) return false;
    if (process.env.NODE_ENV !== "development") {
        throw new Error("A IA fake de demonstração exige NODE_ENV=development.");
    }
    if (!demoMode) {
        throw new Error("Define STUDYFLOW_DEMO_MODE=true para a demonstração.");
    }
    if (!fakeAi) {
        throw new Error("Define STUDYFLOW_DEMO_FAKE_AI=true para a demonstração.");
    }
    return true;
}
