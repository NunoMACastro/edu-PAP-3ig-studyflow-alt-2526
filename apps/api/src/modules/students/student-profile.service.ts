...
export class StudentProfileService {
    ...
    /**
     * Resolve nomes públicos de alunos em batch sem devolver email ou outros
     * campos do perfil. IDs inválidos são ignorados e perfis em falta recebem
     * um pseudónimo estável derivado do próprio identificador.
     *
     * @param userIds Identificadores de utilizador potencialmente repetidos.
     * @returns Mapa de identificador para nome ou pseudónimo seguro.
     */
    async resolvePublicDisplayNames(
        ...
        const profiles = await this.profileModel
            .find({ userId: { $in: validIds.map((id) => new Types.ObjectId(id)) } })
            .select("userId name")
            .lean();
        const names = new Map(
        ...
        for (const userId of validIds) {
            if (!names.get(userId)) names.set(userId, this.fallbackDisplayName(userId));
        }
        return names;
    }

    /** Cria um pseudónimo sem incorporar email, nome ou outro dado pessoal. */
    private fallbackDisplayName(userId: string): string {
        return `Aluno ${userId.slice(-4).toUpperCase()}`;
    }
