...
export class ExternalKnowledgeAiService {
    ...
    async ask(
        ...
        const document = await this.answerModel.create({
            ...
            internalCitations: generated.citations,
     ...
     * @param externalAllowed Decisão final da policy para contexto externo.
     * @returns Resposta validada e citações efetivamente limitadas pela policy.
     */
    ...
    /**
     * Constrói o prompt externo apenas com as citações autorizadas pela fachada.
     *
     * @param areaName Nome da área privada.
     * @param question Pergunta normalizada do aluno.
     * @param citations Citações internas limitadas pela política administrativa.
     * @param externalAllowed Decisão explícita sobre conhecimento externo.
     * @returns Prompt pronto para validação de tamanho e envio.
     */
    private buildPrompt(
        areaName: string,
        question: string,
        citations: readonly ExternalKnowledgeInternalCitation[],
        externalAllowed: boolean,
    ): string {
        return [
            ...
            "Nunca apresentes nota externa como citação interna verificável.",
            `Área: ${areaName}`,
            "Pergunta:",
            question.trim(),
            "Fontes internas:",
