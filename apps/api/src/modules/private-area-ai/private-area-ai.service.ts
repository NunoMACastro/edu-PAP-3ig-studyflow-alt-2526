        ...
        try {
            const execution = await this.aiExecution.executeAuthorized(
                authorization,
                {
                    ...
                    buildPrompt: (selectedSources, conversationHistory) =>
            ...
            const answer = await this.answerModel.create({
                ...
                ...(executionContext
                    ...
                    : {}),
                citationSnapshots: selectedSources
                    .filter((source) => result.sourceMaterialIds.includes(source.materialId))
                    .map((source) => ({
                        label: source.title,
            ...
            if (!assistantContext && executionContext) {
                await this.legacyConversationService?.markAnswered(
                    ...
                    (answer.toObject() as { createdAt?: Date }).createdAt,
                );
            }
