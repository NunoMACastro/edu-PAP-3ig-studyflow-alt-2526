...
describe("TeacherAiVoiceService", () => {
    ...
    it("resolve uma sala sem disciplina diretamente pela voz base da turma", async () => {
        const { classVoiceModel, service, voiceModel } = makeService();
        classVoiceModel.findOne.mockReturnValue(
            leanResult({
        ...
        await expect(
            service.resolveTeacherVoice({ classId }),
        ).resolves.toMatchObject({
            scope: "CLASS",
            source: "CLASS_BASE",
            classId,
        ...
        expect(voiceModel.findOne).not.toHaveBeenCalled();
    });

    it("usa defaults quando turma e disciplina não têm configuração", async () => {
        const { classVoiceModel, service, voiceModel } = makeService();
        voiceModel.findOne.mockReturnValue(leanResult(null));
        classVoiceModel.findOne.mockReturnValue(leanResult(null));

        await expect(
            service.resolveTeacherVoice({ classId, subjectId }),
        ).resolves.toMatchObject({
            scope: "SUBJECT",
            source: "DEFAULT",
            classId,
