...
export type AiConsentCapabilityState =
    | "CURRENT"
    | "OUTDATED"
    | "MISSING"
    | "REVOKED";
...
export type AiConsentCapabilityView = {
    ...
    state: AiConsentCapabilityState;
    canUse: boolean;
    lastDecision?: ReturnType<AiConsentsService["toView"]>;
};
...
export class AiConsentsService {
    ...
    /**
     * Devolve a capacidade efetiva de cada finalidade de IA numa única leitura.
     * A UI não precisa de conhecer nem codificar versões de política.
     */
    async listCapabilities(actor: AuthenticatedUser): Promise<AiConsentCapabilityView[]> {
        const consents = await this.consentModel
            .find({ userId: new Types.ObjectId(actor.id) })
            .sort({ createdAt: -1 })
            .lean();
        const latestByPurpose = new Map<AiConsentPurpose, (typeof consents)[number]>();
        for (const consent of consents) {
        ...
        return (Object.keys(CURRENT_AI_POLICY_VERSIONS) as AiConsentPurpose[]).map(
            (purpose) => {
                const requiredVersion = CURRENT_AI_POLICY_VERSIONS[purpose];
                const consent = latestByPurpose.get(purpose);
                const state: AiConsentCapabilityState = !consent
                    ? "MISSING"
                    : consent.status === "REVOKED"
                      ? "REVOKED"
                      : consent.policyVersion === requiredVersion
                        ? "CURRENT"
                        : "OUTDATED";
                return {
                    purpose,
                    ...
                    ...(consent ? { lastDecision: this.toView(consent) } : {}),
                };
            },
        );
    }
    ...
    async grant(actor: AuthenticatedUser, purpose: AiConsentPurpose, input: UpsertAiConsentDto) {
        ...
        if (
            input.policyVersion &&
            input.policyVersion !== requiredVersion
        ) {
    ...
    async assertGranted(userId: string, purpose: AiConsentPurpose): Promise<void> {
        ...
        if (consent.policyVersion !== CURRENT_AI_POLICY_VERSIONS[purpose]) {
            throw new ForbiddenException({
