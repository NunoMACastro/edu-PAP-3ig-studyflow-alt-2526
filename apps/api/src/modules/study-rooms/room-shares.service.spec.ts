...
describe("RoomSharesService", () => {
    ...
    it("guarda um snapshot Markdown com título, conteúdo e revisão imutáveis", async () => {
        const { materialsService, shareModel, service } = makeService();
        materialsService.findOwnedMaterialReference.mockResolvedValue({
            _id: materialId,
            title: "Resumo oficial do aluno",
            type: "MARKDOWN",
            status: "READY",
            contentText: "# Revisão 2\n\nConteúdo partilhado.\n",
            contentRevision: 2,
        });
        shareModel.create.mockImplementation(async (input) => ({
            ...input,
            _id: shareId,
            toObject() {
                return this;
        ...
        await service.createShare(student, roomId, {
            type: "MATERIAL_REF",
            title: "Título alterado no formulário",
            materialId,
        });
        ...
        expect(shareModel.create).toHaveBeenCalledWith(
            expect.objectContaining({
                title: "Resumo oficial do aluno",
                textContent: "# Revisão 2\n\nConteúdo partilhado.\n",
                contentFormat: "MARKDOWN",
                materialContentRevision: 2,
                usableByAi: true,
            }),
        );
    });
    ...
    it("devolve partilha eliminada sem autor, conteúdo ou referência IA", async () => {
        const { shareModel, service } = makeService();
        shareModel.find.mockReturnValue(
            sortLeanResult([
                {
        ...
        await expect(service.listRoomShares(student, roomId)).resolves.toEqual([
            expect.objectContaining({
