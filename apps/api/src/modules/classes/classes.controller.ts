...
import { ListClassStatusDto } from "./dto/list-class-status.dto.js";
...
export class ClassesController {
    ...
    update(
        ...
        return this.classesService.updateClass(request.user!, classId, body);
    }

    /** Arquiva ou restaura uma turma sem destruir o histórico. */
    @Patch("teacher/classes/:classId/status")
    updateStatus(
        @Req() request: AuthenticatedRequest,
        @Param("classId") classId: string,
        @Body() body: UpdateClassStatusDto,
    ) {
        return this.classesService.updateClassStatus(request.user!, classId, body);
    }
    ...
    listStudent(
        @Req() request: AuthenticatedRequest,
        @Query() query: ListClassStatusDto,
    ) {
        return this.classesService.listStudentClasses(
            request.user!,
            query.status ?? "ACTIVE",
        );
    }
