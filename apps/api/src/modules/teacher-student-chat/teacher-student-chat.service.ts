...
export class TeacherStudentChatService {
    ...
    async listStudentMessages(
        ...
        const access = await this.resolveStudentAccess(actor.id, subjectId);
    ...
    async listTeacherMessages(
        ...
        const access = await this.resolveTeacherAccess(actor.id, subjectId);
        return this.listMessagesForAccess(access);
    }
    ...
    private async listMessagesForAccess(
        ...
        return messages.reverse().map((message) => this.toMessageView(message));
    }
    ...
    private toMessageView(message: {
        ...
        createdAt?: Date;
        tombstonedAt?: Date;
    }): TeacherStudentChatMessageView {
        return {
            _id: String(message._id),
            threadId: String(message.threadId),
            subjectId: String(message.subjectId),
            classId: String(message.classId),
            authorUserId: message.authorUserId ? String(message.authorUserId) : null,
            authorRole: message.authorRole ?? null,
            text: message.text ?? null,
            tombstoned: Boolean(message.tombstonedAt),
