...
import {
    ...
    AiQuotaDefaultPolicyDocument,
} from "./schemas/ai-quota-default-policy.schema.js";
...
export class AiQuotasService {
     ...
     * @param defaultPolicyModel Defaults técnicos aplicados quando não existe uma política específica para o alvo.
    ...
    constructor(
        ...
        @InjectModel(AiQuotaDefaultPolicy.name)
        private readonly defaultPolicyModel: Model<AiQuotaDefaultPolicyDocument>,
    ...
    async reserveUsage(input: ReserveAiUsageDto) {
        ...
        const defaultPolicy = policy
            ? null
            : await this.defaultPolicyModel
                  .findOne({ scope: input.scope, purpose: input.purpose })
                  .lean();
        const monthlyLimitUnits =
            policy?.monthlyLimitUnits ?? defaultPolicy?.monthlyLimitUnits;
        if (monthlyLimitUnits === undefined) {
            throw new ServiceUnavailableException({
                code: "AI_QUOTA_POLICY_NOT_CONFIGURED",
                message: "Esta finalidade de IA ainda não tem quota administrativa.",
            });
        }
        const limit = monthlyLimitUnits;
