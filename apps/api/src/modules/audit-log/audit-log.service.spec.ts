...
describe("AuditLogService", () => {
    ...
    it("mantém a redação recursiva na última barreira antes da BD", () => {
        const { service } = makeService();
        expect(
            service.redactMetadata({
                nested: [{ password: "privada", technicalCount: 2 }],
            }),
        ).toEqual({
            nested: [{ password: "[REDACTED]", technicalCount: 2 }],
        });
    });
});
