...
export class StudyGroupsService {
    ...
    /**
     * Adiciona um aluno por email aplicando internamente o tipo de colaboração
     * e a política owner-only; nenhum destes controlos é escolhido pelo cliente.
     */
    async addMember(
        actor: AuthenticatedUser,
        groupId: string,
        ...
        const room = await this.studyRoomsService.addMember(actor, groupId, input, {
            collaborationKind: "STUDY_GROUP",
            ownerOnly: true,
        });
        return this.toGroupView(room);
