...
export class StudentExperienceService {
    ...
    async getToday(actor: AuthenticatedUser): Promise<StudentTodayState> {
        this.assertStudent(actor);
        const [classes, plan, sessions, guidedPage, recentContexts, unreadChats] =
            await Promise.all([
                this.classesService.listStudentClasses(actor, "ACTIVE"),
                this.routinesService.listMine(actor.id),
                this.groupSessionsService.listUpcomingForStudent(actor),
        ...
        const priorities: StudentAction[] = [
            ...
            ...plan.routines.map((routine) =>
                this.action({
                    ...
                    kind: "ROUTINE",
                    title: routine.title,
                    contextLabel: `${routine.weekdays.join(", ")} · ${routine.startTime}`,
            ...
            ...sessions.map((session) =>
                this.action({
                    ...
                    kind: "GROUP_SESSION",
                    title: session.title,
                    contextLabel: "Sessão de grupo",
                    dueAt: session.startsAt,
                    targetPath: `/app/grupos/${session.groupId}/sessoes`,
                }),
            ),
    ...
    private async resolveContext(
        ...
        if (kind === "STUDY_ROOM" || kind === "STUDY_GROUP") {
            ...
            return this.availableContext(kind, contextId, title, kind === "STUDY_GROUP" ? "Grupo" : "Sala partilhada", kind === "STUDY_GROUP" ? `/app/grupos/${contextId}` : `/app/salas/${contextId}`);
        }
        if (kind === "GUIDED_ROOM") {
