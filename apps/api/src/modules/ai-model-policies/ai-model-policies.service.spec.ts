...
describe("AiModelPoliciesService", () => {
    ...
    it("falha fechada quando a finalidade não tem política persistida", async () => {
        const { policyFindOneLean, service } = makeService();
        policyFindOneLean.mockResolvedValueOnce(null);

        await expect(service.resolveForUse("ROOM_AI")).rejects.toMatchObject({
            response: { code: "AI_MODEL_POLICY_NOT_CONFIGURED" },
        });
    });
...
/**
 * Executa o apoio de teste para políticas de modelos de IA, mantendo o cenário legível e próximo do comportamento real validado.
