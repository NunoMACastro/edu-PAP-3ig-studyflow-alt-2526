...
export class MaterialIndexController {
    ...
    /** Reidrata todos os materiais da área num único pedido autorizado. */
    @Get("student/study-areas/:studyAreaId/material-index-jobs")
    findLatestPrivateJobs(
        @Req() request: AuthenticatedRequest,
        @Param("studyAreaId") studyAreaId: string,
        @Query("latestByMaterial") latestByMaterial?: string,
    ) {
        if (latestByMaterial !== "true") {
            throw new BadRequestException({
                code: "LATEST_BY_MATERIAL_REQUIRED",
                message: "Indica latestByMaterial=true.",
            });
        }
        return this.indexService.listLatestOwnedJobsForArea(
            request.user!,
            studyAreaId,
        );
    }
    ...
    indexOfficial(
        ...
        return this.queueService.enqueueOfficialMaterial({
            actor: request.user!,
            materialId,
        });
    }

    /** Reidrata o estado mais recente dos materiais oficiais da disciplina. */
    @Get("teacher/subjects/:subjectId/material-index-jobs")
    findLatestOfficialJobs(
        @Req() request: AuthenticatedRequest,
        ...
        return this.indexService.listLatestOwnedJobsForSubject(
            request.user!,
            subjectId,
        );
    }
    ...
    findJob(@Req() request: AuthenticatedRequest, @Param("jobId") jobId: string) {
        return this.indexService.findOwnedJobStatus(request.user!, jobId);
    }

    /**
     * Devolve o job mais recente do material para reidratar polling/versões após reload.
     */
    @Get("student/study-areas/:studyAreaId/materials/:materialId/index-jobs/latest")
    findLatestPrivateJob(
        @Req() request: AuthenticatedRequest,
        @Param("studyAreaId") studyAreaId: string,
        @Param("materialId") materialId: string,
    ) {
        return this.indexService.findLatestOwnedJobForMaterial(
            request.user!,
            studyAreaId,
            materialId,
        );
