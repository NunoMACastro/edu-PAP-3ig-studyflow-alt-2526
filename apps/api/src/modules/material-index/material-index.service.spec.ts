...
describe("MaterialIndexService", () => {
    ...
    it("cria job oficial QUEUED idempotente sem expor chunks", async () => {
        const { jobModel, officialMaterialsService, service } = makeService();
        officialMaterialsService.findOwnedMaterialForIndexing.mockResolvedValueOnce({
            _id: materialId,
            subjectId,
            title: "Manual PDF",
            type: "PDF",
            status: "PENDING_PROCESSING",
            storageKey: "users/professor/manual.pdf",
        });

        await expect(
            service.createQueuedOfficialJob(teacher, materialId),
        ).resolves.toMatchObject({
        ...
        expect(jobModel.create).toHaveBeenCalledWith(
            expect.objectContaining({
                activeKey: `official:${teacher.id}:${subjectId}:${materialId}`,
                status: "QUEUED",
            }),
        );
    });

    it("processa PDF oficial reclamado e só então o disponibiliza para IA", async () => {
        const {
            documentSafety,
        ...
        officialMaterialsService.readStoredFile.mockResolvedValueOnce(buffer);
        documentSafety.parseDocument.mockResolvedValueOnce(
            "Texto oficial suficientemente longo para a IA.",
        );

        await service.processClaimedOfficialJob({
            _id: jobId,
            materialId,
            subjectId,
        ...
        expect(documentSafety.assertSafeStoredDocument).toHaveBeenCalledWith(
            expect.objectContaining({ type: "PDF", byteLength: buffer.byteLength }),
        );
        expect(officialMaterialsService.markIndexedText).toHaveBeenCalledWith(
            teacher.id,
            materialId,
        ...
        expect(subjectsService.reserveActiveChildMutation).toHaveBeenCalled();
    });

    it("rejeita o mesmo utilizador quando a role já não corresponde ao scope", async () => {
        const { jobModel, service } = makeService();
        ...
        await expect(
            service.findDoneJob(teacher, jobId),
    ...
    it("revalida disciplina ACTIVE e reserva o mesmo contexto na transação", async () => {
        const { jobModel, officialMaterialsService, service, subjectsService } =
            makeService();
        const contextJob = {
            _id: jobId,
            scope: "OFFICIAL_SUBJECT" as const,
            materialId,
            subjectId,
            teacherId: teacher.id,
        ...
        jobModel.findById.mockReturnValueOnce({
            lean: jest.fn().mockResolvedValue(contextJob),
        });
        officialMaterialsService.findOwnedMaterial.mockResolvedValueOnce({
            _id: materialId,
            subjectId,
        ...
        const context = await service.findWritableDoneJob(teacher, jobId);
        const session = { id: "material-write-session" };
        await service.reserveWritableJob(teacher, context, session as never);

        expect(subjectsService.findOwnedSubject).toHaveBeenCalledWith(
            teacher.id,
            subjectId,
        );
        expect(subjectsService.reserveActiveChildMutation).toHaveBeenCalledWith(
            teacher.id,
    ...
    it("não concede escrita derivada quando a disciplina já está arquivada", async () => {
        const { jobModel, officialMaterialsService, service, subjectsService } =
            makeService();
        jobModel.findById.mockReturnValueOnce({
            lean: jest.fn().mockResolvedValue({
        ...
        subjectsService.findOwnedSubject.mockRejectedValueOnce({
            response: { code: "SUBJECT_NOT_ACTIVE" },
        });

        await expect(
            service.findWritableDoneJob(teacher, jobId),
        ).rejects.toMatchObject({
            response: { code: "SUBJECT_NOT_ACTIVE" },
        });
    });
...
function makeService(
    ...
    const officialMaterialsService = {
        ...
        findOwnedMaterialForIndexing: jest.fn(),
        markIndexedText: jest.fn(),
        readStoredFile: jest.fn(),
    };
