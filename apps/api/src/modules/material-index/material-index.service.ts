...
import { type SubjectView, SubjectsService } from "../subjects/subjects.service.js";
...
/** Contexto revalidado para POST/PATCH derivados de um job concluído. */
export type WritableMaterialIndexJob = {
...
/** Job oficial reclamado sem conservar a sessão HTTP em memória. */
export type ClaimedOfficialMaterialIndexJob = {
    _id: string;
    materialId: string;
    subjectId: string;
    teacherId: string;
...
export class MaterialIndexService {
    private readonly logger = new Logger(MaterialIndexService.name);
    /**
    ...
    async createQueuedPrivateJob(
        ...
        this.assertMaterialNeedsIndexing(material.type);
    ...
    /** Cria ou reutiliza o job oficial ativo sem executar parsing no pedido HTTP. */
    async createQueuedOfficialJob(
        actor: AuthenticatedUser,
        materialId: string,
    ): Promise<MaterialIndexJobView> {
        ...
        const material = (await this.officialMaterialsService.findOwnedMaterialForIndexing(
            actor.id,
            materialId,
        )) as IndexableOfficialMaterial;
        this.assertMaterialNeedsIndexing(material.type);
        await this.subjectsService.findOwnedSubject(actor.id, material.subjectId);
        const activeKey = this.officialActiveKey(
            actor.id,
            material.subjectId,
            materialId,
        );
        const activeJob = await this.jobModel
            .findOne({
                ...
                $or: [
                    ...
                    {
                        materialId: new Types.ObjectId(materialId),
                        subjectId: new Types.ObjectId(material.subjectId),
            ...
            .sort({ createdAt: -1 })
            .lean();
        if (activeJob) return this.toPublicStatusView(this.toView(activeJob));
        ...
        try {
            ...
            const concurrentJob = await this.jobModel
                ...
                .lean();
            if (!concurrentJob) throw error;
            return this.toPublicStatusView(this.toView(concurrentJob));
        }
        await this.recordOfficialIndexAudit(
            actor.id,
            materialId,
            material.subjectId,
            "OFFICIAL_MATERIAL_INDEX_REQUESTED",
            "SUCCESS",
        );
        return this.toPublicStatusView(this.toView(job.toObject()));
    }
    ...
    /**
     * Revalida role, ownership e lifecycle atuais antes de uma escrita derivada.
     * A identidade gravada no job, por si só, não conserva poderes depois de
     * uma mudança de role ou do arquivo do contexto académico.
     */
    async findWritableDoneJob(
        actor: AuthenticatedUser,
        jobId: string,
    ): Promise<WritableMaterialIndexJob> {
        const job = await this.findDoneJob(actor, jobId);
        if (job.scope === "PRIVATE_AREA") {
            if (actor.role !== "STUDENT" || !job.studyAreaId || !job.userId) {
                throw this.roleRequired("STUDENT");
            }
        ...
        if (actor.role !== "TEACHER" || !job.subjectId || !job.teacherId) {
            throw this.roleRequired("TEACHER");
        }
        const [subject, material] = await Promise.all([
            this.subjectsService.findOwnedSubject(actor.id, job.subjectId),
            this.officialMaterialsService.findOwnedMaterial(
                actor.id,
                job.materialId,
            ),
        ]);
        if (material.subjectId !== subject._id) throw this.accessDenied();
        return { job, subject };
    }

    /** Reserva o parent fence académico na transação da escrita derivada. */
    async reserveWritableJob(
        actor: AuthenticatedUser,
        context: WritableMaterialIndexJob,
        session: ClientSession,
    ): Promise<void> {
        if (context.job.scope !== "OFFICIAL_SUBJECT") return;
        if (!context.subject) throw this.roleRequired("TEACHER");
        await this.subjectsService.reserveActiveChildMutation(
            actor.id,
            context.subject.classId,
            context.subject._id,
            session,
        );
    }
    ...
    /** O polling oficial nunca expõe chunks extraídos ao browser. */
    async findOwnedJobStatus(
        actor: AuthenticatedUser,
        jobId: string,
    ): Promise<MaterialIndexJobView> {
        const view = await this.findOwnedJob(actor, jobId);
        return view.scope === "OFFICIAL_SUBJECT"
            ? this.toPublicStatusView(view)
            : view;
    }
    ...
    /** Devolve um resumo por material oficial da disciplina autorizada. */
    async listLatestOwnedJobsForSubject(
        actor: AuthenticatedUser,
        subjectId: string,
    ): Promise<MaterialIndexJobView[]> {
        if (actor.role !== "TEACHER") {
        ...
        await this.subjectsService.findOwnedSubject(actor.id, subjectId);
        ...
        const jobs = await this.jobModel
            .find({
                ...
                teacherId: new Types.ObjectId(actor.id),
                materialId: {
                    $in: materials.map((material) =>
                        new Types.ObjectId(material._id),
                    ),
                },
            })
            .sort({ createdAt: -1, _id: -1 })
        ...
        const latest = new Map<string, MaterialIndexJobView>();
        for (const job of jobs) {
            const view = this.toPublicStatusView(this.toView(job));
            if (!latest.has(view.materialId)) latest.set(view.materialId, view);
        }
        return [...latest.values()];
    }

    /** Resolve internamente os jobs oficiais legíveis por um aluno inscrito. */
    async listLatestReadableJobsForStudentSubject(
        actor: AuthenticatedUser,
        subjectId: string,
    ): Promise<MaterialIndexJobView[]> {
        if (actor.role !== "STUDENT") throw this.accessDenied();
        await this.subjectsService.findSubjectForStudent(actor.id, subjectId);
        const page = await this.officialMaterialsService.listStudentSubjectMaterials(
            actor,
            subjectId,
            undefined,
            50,
        ...
        return [...latest.values()].slice(0, 50);
    }
    ...
    /** Reclama atomicamente o próximo job oficial recuperável. */
    async claimNextOfficialJob(
        leaseOwner: string,
        now = new Date(),
        leaseMs = 30_000,
    ): Promise<ClaimedOfficialMaterialIndexJob | null> {
        await this.jobModel.updateMany(
            {
                scope: "OFFICIAL_SUBJECT",
                $expr: { $gte: ["$attempts", "$maxAttempts"] },
            ...
            },
            {
                $set: {
                    status: "FAILED",
                    errorMessage:
                        "Não foi possível indexar o material oficial neste momento.",
                    completedAt: now,
                },
                $unset: {
                    leaseOwner: "",
        ...
        const claimed = await this.jobModel
            ...
            .lean();
        if (!claimed || !claimed.subjectId || !claimed.teacherId) return null;
        return {
            _id: String(claimed._id),
            materialId: String(claimed.materialId),
            subjectId: String(claimed.subjectId),
            teacherId: String(claimed.teacherId),
            attempts: claimed.attempts ?? 1,
            maxAttempts: claimed.maxAttempts ?? 3,
            leaseOwner,
            leaseToken: claimed.leaseToken ?? 1,
    ...
    /** Processa um job oficial sem conservar autoridade da sessão original. */
    async processClaimedOfficialJob(
        claimed: ClaimedOfficialMaterialIndexJob,
    ): Promise<void> {
        try {
            const { material, subject, extraction } =
                await executeWithMongoLeaseHeartbeat({
                    leaseMs: claimed.leaseMs,
                    heartbeat: () => this.renewOfficialJobLease(claimed),
                    operation: async () => {
                        if (this.usersService) {
                            const current = await this.usersService.findSessionUser(
                                claimed.teacherId,
                            );
                            if (!current || current.user.role !== "TEACHER") {
                                throw new Error("Professor sem autoridade atual.");
                            }
                        }
                        const material = (await this.officialMaterialsService.findOwnedMaterialForIndexing(
                            claimed.teacherId,
                        ...
                        if (material.subjectId !== claimed.subjectId) {
                            throw new Error("Material fora da disciplina do job.");
                        }
                        const subject = await this.subjectsService.findOwnedSubject(
                            claimed.teacherId,
                            claimed.subjectId,
                        ...
                        const extraction = await this.extractOfficialMaterial(material);
                        return { material, subject, extraction };
                    },
                });
            const completedAt = new Date();
            await this.runInTransaction(async (session) => {
                ...
                await this.jobModel.updateOne(
                    this.officialLeaseFilter(claimed, completedAt),
                    {
                        $set: {
                            status: extraction.text ? "DONE" : "FAILED",
                            extractedTextChunks: this.createChunks(
                            ...
                            ...(extraction.text
                                ...
                                : {
                                      errorMessage:
                                          extraction.errorMessage ??
                                          "O material não tem texto processável disponível.",
                                  }),
                            completedAt,
                        },
                        $unset: {
            ...
            await this.recordOfficialIndexAudit(
                ...
                claimed.subjectId,
                extraction.text
                    ? "OFFICIAL_MATERIAL_INDEX_COMPLETED"
                    : "OFFICIAL_MATERIAL_INDEX_FAILED",
                extraction.text ? "SUCCESS" : "FAILED",
            );
        } catch (error) {
            if (error instanceof MongoLeaseLostError) return;
            const exhausted = claimed.attempts >= claimed.maxAttempts;
            const transitionAt = new Date();
            await this.jobModel.updateOne(
                this.officialLeaseFilter(claimed, transitionAt),
                exhausted
                    ? {
                          $set: {
                              status: "FAILED",
                    ...
                    : {
                          $set: {
                              ...
                              errorMessage:
                                  "Falha transitória; a indexação será repetida.",
                              availableAt: new Date(
                                  Date.now() + this.retryDelayMs(claimed.attempts),
                              ),
                          },
                          $unset: { leaseOwner: "", leaseExpiresAt: "" },
                      },
    ...
    private async renewOfficialJobLease(
        claimed: ClaimedOfficialMaterialIndexJob,
    ): Promise<boolean> {
        const now = new Date();
        const result = await this.jobModel.updateOne(
            this.officialLeaseFilter(claimed, now),
            {
                $set: {
    ...
    private assertOwnedJob(
        ...
        if (
            (view.scope === "PRIVATE_AREA" &&
                (actor.role !== "STUDENT" || view.userId !== actor.id)) ||
            (view.scope === "OFFICIAL_SUBJECT" &&
                (actor.role !== "TEACHER" || view.teacherId !== actor.id))
        ) {
    ...
    private async createJob(input: {
        ...
        errorMessage?: string;
    }, session?: ClientSession): Promise<MaterialIndexJobView> {
        const base = {
            scope: input.scope,
        ...
        const document = input.chunks.length === 0
                ? {
                      ...base,
                      status: "FAILED",
                      errorMessage:
        ...
        const job = session
            ? (await this.jobModel.create([document], { session }))[0]
            : await this.jobModel.create(document);
        return this.toView(job.toObject());
    }

    /** Mantém o fence académico, a projeção do material e o job no mesmo commit. */
    ...
    private async extractOfficialMaterial(
        ...
        try {
            ...
            if (
                (material.type === "PDF" || material.type === "DOCX") &&
                material.storageKey
            ) {
                const buffer = await this.officialMaterialsService.readStoredFile(
                    material.storageKey,
                );
                this.documentSafety.assertSafeStoredDocument({
                    type: material.type,
    ...
    private roleRequired(role: "STUDENT" | "TEACHER"): ForbiddenException {
        return new ForbiddenException({
    ...
    /** Impede que Markdown, já textual e pronto para IA, entre em jobs ou versões. */
    private assertMaterialNeedsIndexing(type: string): void {
        if (type === "MARKDOWN") {
            throw new BadRequestException({
                code: "MARKDOWN_INDEX_NOT_REQUIRED",
                message: "Documentos Markdown não necessitam de indexação.",
            });
        }
    }
    ...
    private officialActiveKey(
        teacherId: string,
        subjectId: string,
        materialId: string,
    ): string {
        return `official:${teacherId}:${subjectId}:${materialId}`;
    }

    private toPublicStatusView(
        view: MaterialIndexJobView,
    ): MaterialIndexJobView {
        return { ...view, extractedTextChunks: [] };
    }

    private async recordOfficialIndexAudit(
        teacherId: string,
        materialId: string,
        subjectId: string,
        action:
        ...
        try {
            ...
            this.logger.warn("Falha controlada ao auditar indexação oficial.");
        }
    }
