...
export class MaterialIndexService {
            ...
            if (material.type === "PDF") {
                return this.toReadableExtraction(
                    await this.documentSafety.runWithTimeout({
                        ...
                        /**
                         * Executa operation no domínio de indexação segura de materiais, aplicando validações, autorização e persistência de forma coesa.
                         *
                         * @returns Resultado da operação no formato esperado pelo chamador.
                         */
     ...
     * Este fluxo valida ficheiros, URLs ou fontes externas para reduzir riscos de entrada não confiável, SSRF ou DNS rebinding.
     *
     * @param buffer Valor de buffer usado pela função para executar extract pdf text com dados explícitos.
     * @returns Texto normalizado, identificador ou conteúdo seguro produzido pela operação.
     */
     ...
     * Este fluxo valida ficheiros, URLs ou fontes externas para reduzir riscos de entrada não confiável, SSRF ou DNS rebinding.
     *
     * @param buffer Valor de buffer usado pela função para executar extract docx text com dados explícitos.
     * @returns Texto normalizado, identificador ou conteúdo seguro produzido pela operação.
     */
