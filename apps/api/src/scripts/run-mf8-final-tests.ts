...
export const MF8_HISTORICAL_FINAL_EVIDENCE_PATH = legacyMf8EvidencePath(
    "TESTES-FINAIS.md",
);
const MF8_INVENTORY_EVIDENCE_PATH = legacyMf8EvidencePath(
    "TESTES-EM-FALTA.md",
);
...
export function validateInventoryEvidence(repoRoot: string): InventoryEvidenceCheck {
    ...
    const hasBlockingDecision =
        /N[aã]o avances para a execu[cç][aã]o final/i.test(content) ||
        /Snapshot historico com lacunas/i.test(content);

    return {
        path: evidencePath,
        status: hasBlockingDecision ? "BLOQUEADO" : "PASS",
        observed: hasBlockingDecision
            ? "A evidence historica do BK-MF8-15 indica lacunas P0."
            : "A evidence historica do BK-MF8-15 permite reproduzir o gate MF8, sem autoridade de release.",
    };
}
...
export function renderFinalEvidenceMarkdown(evidence: FinalGateEvidence): string {
    ...
    return [
        "# TESTES-FINAIS - MF8 (historico)",
        "",
        ...legacyMf8FrontMatter(),
        "",
        "Esta bateria reproduz apenas o gate anterior ao manifesto final.",
        "Nao substitui `verify:local-release` e nunca fecha a release atual.",
        "",
        "## Decisao historica",
        "",
        hasBlockingFailure(evidence)
            ? "- BLOQUEADO_HISTORICO: existiam falhas obrigatorias ou evidence de entrada insuficiente."
            : "- PASS_HISTORICO: a bateria MF8 passou naquele snapshot; sem autoridade para a release atual.",
...
export function createMf8FinalEvidence(
    ...
    assertLegacyMf8EvidenceOptIn(environment);
    const evidencePath = resolve(repoRoot, MF8_HISTORICAL_FINAL_EVIDENCE_PATH);
...
export function runMf8FinalTestsCli(): void {
    ...
    process.stdout.write(
        `${JSON.stringify({ ok: !hasBlockingFailure(evidence), scope: "MF8_HISTORICO", output: MF8_HISTORICAL_FINAL_EVIDENCE_PATH })}\n`,
    );
