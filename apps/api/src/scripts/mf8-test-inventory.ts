...
export const MF8_HISTORICAL_INVENTORY_PATH = legacyMf8EvidencePath(
    "TESTES-EM-FALTA.md",
);
 ...
 * @returns Markdown inequivocamente historico, nunca evidence da release atual.
 */
export function renderInventoryMarkdown(summary: TestInventorySummary): string {
    ...
    return [
        "# TESTES-EM-FALTA - MF8 (historico)",
        "",
        ...legacyMf8FrontMatter(),
        "",
        "Este inventario cobre apenas os oito alvos definidos no fecho historico da MF8.",
        "Nao mede a suite integral atual e nao autoriza PASS, release ou prontidao.",
        "",
        ...
        summary.missingSpecs === 0 && summary.missingSources === 0
            ...
            : "- Snapshot historico com lacunas; este resultado nao deve ser promovido a evidence atual.",
        "",
    ].join("\n");
}

/** Escreve o inventario apenas na pasta historica fixa e apos opt-in explicito. */
export function writeHistoricalMf8TestInventory(
    repoRoot = resolve(process.cwd(), "../.."),
    summary = createMf8TestInventory(repoRoot),
    environment: NodeJS.ProcessEnv = process.env,
): string {
    assertLegacyMf8EvidenceOptIn(environment);
    const outputPath = resolve(repoRoot, MF8_HISTORICAL_INVENTORY_PATH);
    mkdirSync(dirname(outputPath), { recursive: true });
    writeFileSync(
        outputPath,
        renderInventoryMarkdown(summary),
        "utf8",
    );
    return outputPath;
}
...
export function runMf8TestInventoryCli(): void {
    ...
    writeHistoricalMf8TestInventory(resolve(process.cwd(), "../.."), summary);
    process.stdout.write(
        `${JSON.stringify({ ok: true, scope: "MF8_HISTORICO", output: MF8_HISTORICAL_INVENTORY_PATH })}\n`,
    );
            ...
            `${JSON.stringify({
                ...
                error:
                    error instanceof Error
                        ? error.message
                        : "Falha no inventario MF8 historico.",
            })}\n`,
        );
        process.exitCode = 1;
    }
