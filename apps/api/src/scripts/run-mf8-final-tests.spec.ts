...
describe("run-mf8-final-tests", () => {
    ...
    it("sanitiza segredos e renderiza a decisão final", () => {
        ...
        expect(markdown).toContain("- Ficheiro: docs/evidence/MF8/TESTES-EM-FALTA.md");
    ...
    it("remove diretoria local dos paths de evidence", () => {
        const path = sanitizeEvidencePath(
            "/Users/nuno/projeto/studyflow_alt/docs/evidence/MF8/TESTES-EM-FALTA.md",
        );

        expect(path).toBe("docs/evidence/MF8/TESTES-EM-FALTA.md");
    });
    ...
    it("permite avançar quando a evidence anterior não tem decisão bloqueante", async () => {
        ...
        try {
            await writeFile(
                join(repoRoot, "docs/evidence/MF8/TESTES-EM-FALTA.md"),
                "- Pode avancar para a execucao final.\n",
                "utf8",
            );
    ...
    it("bloqueia evidence anterior com frase acentuada", async () => {
        ...
        try {
            await writeFile(
                join(repoRoot, "docs/evidence/MF8/TESTES-EM-FALTA.md"),
                "- Não avances para a execução final sem corrigir lacunas P0.\n",
                "utf8",
            );
