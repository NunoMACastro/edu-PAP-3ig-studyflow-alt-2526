...
import {
    ...
    type AiQuotaDefaultPolicySource,
} from "../modules/ai-quotas/schemas/ai-quota-default-policy.schema.js";
import {
    AiArtifact,
    AiArtifactSchema,
...
} from "../modules/student-ai-assistant/schemas/student-ai-conversation.schema.js";
import {
    StudyEvent,
    StudyEventSchema,
...
const CLASS_QUOTA_PURPOSES: AiConsentPurpose[] = ["CLASS_AI"];
const GROUP_QUOTA_PURPOSES: AiConsentPurpose[] = ["GROUP_AI", "ROOM_AI"];
const DEMO_AI_MONTHLY_LIMIT_UNITS = 5_000;
const E2E_AI_MONTHLY_LIMIT_UNITS = 100;
...
export async function seedDevelopmentEnvironment(): Promise<void> {
    ...
    if (e2eGovernance) {
        await seedE2eAiGovernance(userModel, userIds);
    } else {
        await seedDevelopmentAiGovernance(userModel, userIds, {
            grantSyntheticConsents:
...
function createSeedModels() {
    return {
        ...
        studentAiConversation: seedModel(
            StudentAiConversation.name,
            StudentAiConversationSchema,
        ),
        studyGroupAiAnswer: seedModel(
        ...
        studyGroupChatReadState: seedModel(
...
async function seedDemoContent(
    ...
    await seedStudentAssistantConversations(models, studentIds, academic);
    await seedTeacherStudentChat(models, teacherId, studentIds, academic);
}

/** Cria conversas nativas variadas e mantém uma interação legacy sem ligação. */
async function seedStudentAssistantConversations(
    models: SeedModels,
    studentIds: Types.ObjectId[],
    academic: AcademicSeedResult,
): Promise<void> {
    const studentId = studentIds[0];
    const [area, group, room, guidedRoom] = await Promise.all([
        models.studyArea.findOne({ userId: studentId }).lean(),
        models.studyRoom.findOne({
            memberIds: studentId,
        ...
        models.guidedStudyRoom.findOne({ classId: { $in: academic.classIds } }).lean(),
    ]);
    if (
        !area?._id ||
        !group?._id ||
        !room?._id ||
        !guidedRoom?._id ||
        !academic.subjectIds[0]
    ) return;
    const now = new Date();
    const conversationSeeds = [
        { kind: "SUBJECT", contextId: academic.subjectIds[0], label: "Bases de Dados", secondary: "12.º GPSI", title: "Revisão de normalização e transações", status: "ACTIVE" },
        { kind: "SUBJECT", contextId: academic.subjectIds[0], label: "Bases de Dados", secondary: "12.º GPSI", title: "Dúvidas anteriores da disciplina", status: "ARCHIVED" },
        { kind: "STUDY_AREA", contextId: area._id, label: String(area.name ?? "Área pessoal"), secondary: "Estudo pessoal", title: "Explicações passo a passo", status: "ACTIVE" },
        { kind: "STUDY_GROUP", contextId: group._id, label: String(group.name ?? "Grupo de estudo"), secondary: "Grupo", title: "Preparação da próxima sessão", status: "ACTIVE" },
        { kind: "STUDY_ROOM", contextId: room._id, label: String(room.name ?? "Sala partilhada"), secondary: "Sala partilhada", title: "Prioridades do grupo e exemplos detalhados para a próxima sessão de estudo", status: "ACTIVE" },
        { kind: "GUIDED_ROOM", contextId: guidedRoom._id, label: String(guidedRoom.title ?? "Sala guiada"), secondary: "Com o professor", title: "Questões da atividade guiada", status: "ACTIVE" },
        { kind: "SUBJECT", contextId: new Types.ObjectId(), label: "Disciplina anterior", secondary: "Acesso terminado", title: "Histórico de um contexto cujo acesso terminou", status: "ACTIVE" },
    ] as const;
    let groupConversationId: Types.ObjectId | undefined;
    let roomConversationId: Types.ObjectId | undefined;
    let areaConversationId: Types.ObjectId | undefined;
    for (const seed of conversationSeeds) {
        const conversationId = await upsertSeed(
            models.studentAiConversation,
            { studentId, contextKind: seed.kind, contextId: seed.contextId, title: seed.title },
            {
                contextLabelSnapshot: seed.label,
                contextSecondaryLabelSnapshot: seed.secondary,
                status: seed.status,
                origin: "NATIVE",
                readOnly: false,
                lastMessageAt: now,
        ...
        if (seed.kind === "STUDY_ROOM" && seed.status === "ACTIVE") {
            roomConversationId = conversationId;
        }
        if (seed.kind === "STUDY_GROUP" && seed.status === "ACTIVE") {
            groupConversationId = conversationId;
        }
        if (seed.kind === "STUDY_AREA" && seed.status === "ACTIVE") {
            areaConversationId = conversationId;
        }
    }
    if (groupConversationId) {
        const share = await models.roomShare
            .findOne({ roomId: group._id, usableByAi: true, tombstonedAt: null })
            .lean();
        if (share?._id) {
            await upsertSeed(
                models.studyGroupAiAnswer,
                {
                    ...
                    question: "Como priorizamos a próxima sessão de grupo?",
                },
                {
                    answer: "Comecem pela dúvida comum, resolvam um exemplo e terminem com uma tarefa curta por pessoa.",
                    sources: [{ shareId: String(share._id), title: String(share.title) }],
                    citationSnapshots: [{
                        label: String(share.title),
                        kind: "GROUP_RESOURCE",
                    }],
    ...
    if (roomConversationId) {
        await models.roomAiInteraction.updateMany(
            { roomId: room._id, studentId, conversationId: { $exists: false } },
            {
                $set: {
                    conversationId: roomConversationId,
                    citationSnapshots: [{
                        label: "Resumo colaborativo",
                        kind: "ROOM_SHARE",
                    }],
                },
            },
    ...
    if (areaConversationId) {
        await models.aiArtifact.updateMany(
            { userId: studentId, studyAreaId: area._id },
            { $set: { assistantConversationId: areaConversationId } },
        );
        const quiz = await models.aiArtifact
            .findOne({
                userId: studentId,
                studyAreaId: area._id,
                type: "QUIZ",
            })
            .lean();
        const commonJob = {
            userId: studentId,
            studyAreaId: area._id,
            assistantConversationId: areaConversationId,
            attempts: 1,
            maxAttempts: 3,
            availableAt: now,
            leaseToken: 1,
        ...
        if (quiz?._id) {
            await upsertSeed(
                ...
                { assistantRequestKey: `seed-assistant-quiz-done:${studentId}` },
                {
                    ...commonJob,
                    status: "DONE",
                    artifactId: quiz._id,
                    topic: "Revisão concluída",
                    completedAt: now,
                },
            );
        }
        await upsertSeed(
            models.quizGenerationJob,
            { assistantRequestKey: `seed-assistant-quiz-failed:${studentId}` },
            {
                ...commonJob,
                status: "FAILED",
                topic: "Tópico extenso para validar a recuperação explícita depois de uma falha controlada",
                errorMessage: "Não foi possível gerar o quiz neste momento.",
                completedAt: now,
            },
        );
        await upsertSeed(
            models.quizGenerationJob,
            { assistantRequestKey: `seed-assistant-quiz-processing:${studentId}` },
            {
                ...commonJob,
                status: "PROCESSING",
                topic: "Processamento persistente para QA responsivo",
                leaseOwner: "seed-fixture",
                leaseExpiresAt: new Date("2099-01-01T00:00:00.000Z"),
                activeKey: `seed-active-assistant-quiz:${studentId}`,
            },
        );
    }
}
...
async function seedCollaborativeContent(
    ...
    const roomSeeds = [
        { owner: 0, name: "Sprint final de PAP", type: "FREE", disciplineName: undefined, members: [0, 1, 2, 3, 4], kind: "STUDY_GROUP", description: "Planeamento, revisão técnica e apoio mútuo para as entregas finais." },
        { owner: 1, name: "Clube de Node.js", type: "SUBJECT", disciplineName: "Programação", members: [0, 1, 2, 4, 5], kind: "STUDY_GROUP", description: "Dúvidas, exemplos e desafios semanais sobre backend moderno." },
        { owner: 3, name: "SQL sem medo", type: "SUBJECT", disciplineName: "Bases de Dados", members: [0, 3, 6, 7, 8], kind: "STUDY_GROUP", description: "Grupo de prática para modelação, consultas e preparação de testes." },
        { owner: 6, name: "Foco e organização", type: "FREE", disciplineName: undefined, members: [0, 2, 6, 7, 8], kind: "STUDY_GROUP", description: "Sessões curtas, objetivos semanais e partilha de técnicas de estudo." },
        { owner: 0, name: "Sala partilhada E2E", type: "FREE", disciplineName: undefined, members: [0, 1, 2, 3], kind: "STUDY_ROOM", description: "Sala dedicada a validar colaboração privada e forks completos." },
    ] as const;
    for (const [roomIndex, room] of roomSeeds.entries()) {
        ...
        for (const [messageIndex, text] of messages.entries()) {
            const messageId = await upsertSeed(
            ...
            if (messageIndex !== 2) latestMessageId = messageId;
        }
        if (room.kind === "STUDY_GROUP" && latestMessageId && memberIds[1]) {
            await upsertSeed(
                models.studyGroupChatReadState,
                { studentId: memberIds[1], groupId: roomId },
                {
                    lastReadAt: dateFromNow(-1 - roomIndex),
...
async function seedE2eAiGovernance(
    ...
    await seedAiModelPolicies(policyModel, "studyflow-e2e-fake", false);
    await seedAiQuotaConfiguration(
        userIds,
        E2E_AI_MONTHLY_LIMIT_UNITS,
        "E2E_SEED",
    );
...
/**
 * Resolve o modelo administrativo que continuará válido quando a mesma base
 * deixar o modo de demonstração e voltar a usar o provider OpenAI real.
 */
...
/**
 * Ativa governação apenas para as contas sintéticas. O runtime pode usar o
 * provider fake, mas a política persistida conserva o modelo OpenAI real para
 * não inutilizar a base quando o operador regressa ao arranque normal.
 */
async function seedDevelopmentAiGovernance(
    userModel: Model<User>,
    userIds: Map<string, Types.ObjectId>,
    options: { grantSyntheticConsents: boolean },
): Promise<void> {
    const policyModel = mongoose.model(AiModelPolicy.name, AiModelPolicySchema);
    const consentModel = mongoose.model(AiConsent.name, AiConsentSchema);
    await seedAiModelPolicies(policyModel, resolveDemoAiPolicyModel(), true);
    await seedAiQuotaConfiguration(
        userIds,
        DEMO_AI_MONTHLY_LIMIT_UNITS,
        "DEMO_SEED",
    );
...
/** Sincroniza a política de modelo sem depender do provider fake do runtime. */
async function seedAiModelPolicies(
    policyModel: Model<AiModelPolicy>,
    model: string,
    enableRoomAi: boolean,
): Promise<void> {
    for (const purpose of AI_PURPOSES) {
        await policyModel.updateOne(
            ...
            {
                $set: {
                    enabled: purpose !== "ROOM_AI" || enableRoomAi,
                    provider: "openai",
                    model,
                    timeoutMs: 8_000,
                    maxSourceCount: 10,
...
/**
 * Cria defaults para todos os âmbitos da demonstração sem substituir políticas
 * específicas que tenham sido configuradas por um administrador.
 */
async function seedAiQuotaConfiguration(
    userIds: Map<string, Types.ObjectId>,
    ...
    const defaultModel = mongoose.model(
        AiQuotaDefaultPolicy.name,
        AiQuotaDefaultPolicySchema,
    );
    const studentIds = developmentUsers
        .filter(({ role }) => role === "STUDENT")
        .map(({ email }) => requiredUserId(userIds, email));
    const teacherIds = developmentUsers
        .filter(({ role }) => role === "TEACHER")
        .map(({ email }) => requiredUserId(userIds, email));
    const allSeededUserIds = [...studentIds, ...teacherIds];

    await Promise.all(
        quotaDefaultMatrix().map(({ scope, purpose }) =>
            defaultModel.updateOne(
                { scope, purpose },
                {
                    $set: { monthlyLimitUnits, source },
                    $setOnInsert: { scope, purpose },
                },
                { upsert: true, runValidators: true },
            ),
        ),
    ...
    // Remove apenas combinações que a antiga seed criou no alvo errado.
    await quotaModel.deleteMany({
        scope: "USER",
        targetId: { $in: allSeededUserIds },
        purpose: { $in: [...CLASS_QUOTA_PURPOSES, ...GROUP_QUOTA_PURPOSES] },
    });
    await quotaModel.deleteMany({
        scope: "USER",
        targetId: { $in: teacherIds },
        purpose: { $in: USER_QUOTA_PURPOSES },
    });

}

/** Matriz única usada pela seed normal, pelo E2E e pelos testes de contrato. */
export function quotaDefaultMatrix(): Array<{
    ...
    return [
        ...
        ...CLASS_QUOTA_PURPOSES.map((purpose) => ({
            scope: "CLASS" as const,
            purpose,
        })),
        ...GROUP_QUOTA_PURPOSES.map((purpose) => ({
