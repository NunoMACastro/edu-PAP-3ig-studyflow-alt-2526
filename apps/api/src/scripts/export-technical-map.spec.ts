...
describe("exportTechnicalMap", () => {
    it("exporta mapa tecnico com modulos, endpoints, modelos, fluxos e validacao", async () => {
        ...
        expect(markdown).toContain("`StudyGroupMessagesModule`");
        expect(markdown).toContain("`StudentsModule`");
        expect(markdown).toContain("`ClassesModule`");
        expect(markdown).toContain("`SubjectsModule`");
        expect(markdown).toContain("`ClassLearningActivityModule`");
        expect(markdown).toContain("`NotificationOutboxModule`");
        expect(markdown).toContain("`/api/ai/source-grounded-answers`");
        expect(markdown).toContain("`/api/ai-consents/capabilities`");
        expect(markdown).toContain("`/api/student/guided-study-rooms`");
        expect(markdown).toContain("`/api/student/projects/:projectId/ai-plans`");
        expect(markdown).toContain("`/api/student/subjects/:subjectId/materials`");
        expect(markdown).toContain(
            "`/api/follow-up-centre/classes/:classId/students/:studentId`",
        );
        expect(markdown).toContain("`/api/context-notifications/inbox`");
        expect(markdown).toContain("`/api/study-rooms/:roomId/ai/answers`");
        expect(markdown).toContain("`/api/study-groups/:groupId/messages`");
        expect(markdown).toContain("`StudyGroupMessage`");
        expect(markdown).toContain("Chat do grupo de estudo");
        expect(markdown).toContain("`StudentProfile`");
        expect(markdown).toContain("`ClassMembership`");
        expect(markdown).toContain("`ClassLearningActivity`");
        expect(markdown).toContain("`StudentClassActivityState`");
        expect(markdown).toContain("`ContextNotificationRecipient`");
        expect(markdown).toContain("`NotificationOutboxEvent`");
        expect(markdown).toContain("`ApprovedAiQuizAttempt`");
        expect(markdown).not.toContain("nome, ano escolar, curso e turma");
