...
export function loadRuntimeConfig(
    ...
    return {
        ...
        trustProxy: false,
        mongoUri: normaliseLocalMongoUri(
...
/** Exige Mongo loopback com replica set, condição necessária às transações. */
function normaliseLocalMongoUri(rawUri: string): string {
    const uri = new URL(rawUri);
    if (
        uri.protocol !== "mongodb:" ||
        !["127.0.0.1", "localhost", "[::1]"].includes(uri.hostname.toLowerCase())
    ) {
        throw new Error("MONGODB_URI tem de usar MongoDB em loopback no âmbito local-pap.");
    }
    ...
    if (!/^studyflow(?:[_-](?:dev|test|e2e))?$/i.test(databaseName)) {
        throw new Error("MONGODB_URI tem de indicar uma base StudyFlow local permitida.");
    }
    if (!uri.searchParams.get("replicaSet")?.trim()) {
        throw new Error("MONGODB_URI tem de configurar replicaSet para transações.");
    }
    return rawUri;
}
