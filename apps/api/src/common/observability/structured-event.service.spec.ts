...
describe("StructuredEventService", () => {
    ...
    it("redige segredos em objetos e arrays aninhados", () => {
        const event = recordWith({
            request: {
                headers: { authorization: "Bearer privado" },
                turns: [{ prompt: "conteúdo privado", sourceCount: 2 }],
            },
        });
        ...
        expect(event.metadata).toEqual({
            request: {
                ...
                turns: [{ prompt: "[REDACTED]", sourceCount: 2 }],
            },
        });
    });

    it("termina ciclos sem serializar o objeto original", () => {
        const cyclic: Record<string, unknown> = { safe: true };
        cyclic.self = cyclic;
        expect(recordWith({ cyclic }).metadata).toEqual({
            cyclic: { safe: true, self: { value: "[TRUNCATED]" } },
        });
    });
