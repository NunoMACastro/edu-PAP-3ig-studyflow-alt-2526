...
describe("HealthController", () => {
    ...
    it("devolve liveness pública da API com disponibilidade agregada", () => {
    ...
    it("só declara readiness depois de Mongo, Redis e storage responderem", async () => {
    ...
    it("falha readiness com 503 quando uma dependência não responde", async () => {
        const service = new HealthService(
            { db: { command: jest.fn().mockRejectedValue(new Error("offline")) } } as never,
            { ping: jest.fn().mockResolvedValue("PONG") } as never,
            { checkReady: jest.fn().mockResolvedValue(undefined) } as never,
            { checkReady: jest.fn() } as never,
            { checkReady: jest.fn() } as never,
 ...
 *
 * @param key Valor de key usado pela função para executar restore env com dados explícitos.
