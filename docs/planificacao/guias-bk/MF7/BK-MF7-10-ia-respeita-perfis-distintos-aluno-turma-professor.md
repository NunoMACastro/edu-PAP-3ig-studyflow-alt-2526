...
Neste BK vais implementar uma política explícita para separar IA privada, IA de sala/grupo e IA da turma/disciplina. `assertAiContextProfile(...)` falha antes da execução, e o fluxo delega depois na `GovernedAiExecutionService`.
...
- Estado depois: `assertAiContextProfile(...)` é chamado após membership e antes de listar materiais ou invocar a fachada governada.
...
- **Falhar cedo:** bloquear antes de listar fontes ou invocar a fachada.
...
/**
 * Perfis pedagógicos permitidos antes da execução governada.
 */
...
export function assertAiContextProfile(
    ...
    if (profileType !== expectedProfile) {
        // Falhar antes da fachada impede fuga de materiais ou voz docente entre contextos.
...
O código define a matriz contexto/perfil e bloqueia antes de qualquer execução governada receber contexto incorreto.
...
A ordem é intencional: role, membership e perfil/contexto antecedem fontes e fachada. A fachada concentra consentimento, policy, limites, guardrails, reserva atómica de quota, integração externa, validação de output e audit seguro.

6. Validação do passo.

Resultado esperado: membership e perfil falham antes de listar materiais ou invocar `GovernedAiExecutionService`.
...
it("bloqueia perfil incompatível antes de materiais, fachada e persistência", async () => {
    const {
    ...
    // A falha acontece antes de ler materiais, executar IA ou gravar interação.
...
O teste força uma falha controlada e confirma que a app não listou materiais, não chamou a fachada e não gravou interação.
