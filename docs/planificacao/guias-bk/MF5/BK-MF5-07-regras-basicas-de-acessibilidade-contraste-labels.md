...
## Contexto do BK
- Entrega alvo: `Regras básicas de acessibilidade (contraste, labels).` com rastreabilidade direta para `RNF05`.
...
- [ ] Sei explicar como `RNF05` se traduz em comportamento implementável.
...
### Passos
1. Confirmar no backlog e na matriz o escopo de `BK-MF5-07` e do requisito `RNF05`.
...
export function validarFormulario(state: FormState) {
  ...
  return { bkId: 'BK-MF5-07', req: 'RNF05', valido: erros.length === 0, erros };
}
```

Cria feedback imediato e determinístico no fluxo de UI.
- Requisitos alvo deste BK: `RNF05`.
