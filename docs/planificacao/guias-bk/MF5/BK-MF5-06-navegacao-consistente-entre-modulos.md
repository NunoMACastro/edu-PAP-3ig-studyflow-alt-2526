...
Neste BK vais implementar uma navegação declarativa com React Router. As páginas são carregadas com `lazy`, a área autenticada usa `ProtectedLayout`, cada ramo por papel usa `RoleGuard`, e existem páginas explícitas 403, 404 e uma error boundary. Uma rota proibida não monta o componente nem dispara pedidos.

O parâmetro `returnTo` aceita exclusivamente paths internos iniciados por `/`, sem protocolo, host ou `//`; qualquer valor externo cai no destino autenticado seguro. A navegação usa os roles reais (`STUDENT`, `TEACHER`, `ADMIN`) e `NavLink` marca a página atual com `aria-current="page"`.
...
- Criar `apps/web/src/components/layout/navigation.ts` e `apps/web/src/routes/AppRouter.tsx`.
- Atualizar `apps/web/src/components/layout/AppShell.tsx` para `NavLink` e disclosure mobile.
- Usar os roles reais de `User.role`: `STUDENT`, `TEACHER`, `ADMIN`.
- Migrar as rotas existentes para `Routes`, `ProtectedLayout` e `RoleGuard`, sem inventar endpoints.
- Marcar o link ativo com `aria-current="page"`.
- Criar smoke Playwright focado em navegação por role e link ativo.

#### Scope-out

- Criar funcionalidades de produto novas; 403/404 são estados técnicos obrigatórios.
...
`navigation.ts` define todos os itens de navegação autenticada. `AppRouter.tsx` é a fonte única de rotas; usa `Suspense`, imports lazy por rota/papel, `ProtectedLayout`, `RoleGuard`, 403, 404 e error boundary. `AppShell.tsx` usa `NavLink`, pelo que não lê diretamente o URL nem duplica o router. O smoke E2E entra como aluno e professor pela UI, valida cookies HttpOnly, confirma que uma rota proibida não monta nem chama a API e rejeita `returnTo` externo.
...
Remove os arrays locais, usa `NavLink` e implementa o menu mobile como disclosure abaixo do header. O botão e os links têm alvo mínimo de 44 px; Escape, clique exterior e navegação fecham o disclosure, e Escape devolve foco ao botão.
...
export function AppShell({ user, children, onLogout }: AppShellProps) {
    ...
    useEffect(() => {
        ...
        const closeFromInteraction = (event: KeyboardEvent | PointerEvent) => {
        ...
        document.addEventListener("keydown", closeFromInteraction);
        document.addEventListener("pointerdown", closeFromInteraction);
        return () => {
            document.removeEventListener("keydown", closeFromInteraction);
            document.removeEventListener("pointerdown", closeFromInteraction);
        };
    }, [mobileOpen]);
        ...
        <div className="min-h-screen bg-slate-50">
            ...
            <header className="border-b border-slate-200 bg-white">
                <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3">
                    <NavLink to="/app/estudo" className="text-xl font-bold text-teal-800">
                        StudyFlow
                    </NavLink>
                    <nav aria-label="Navegação principal" className="hidden items-center gap-2 md:flex">
                        {navigation.map((item) => (
                            <NavLink
                                className={({ isActive }) =>
                                    isActive
                                        ? "flex min-h-11 items-center rounded-md bg-teal-50 px-3 font-semibold text-teal-800"
                                        : "flex min-h-11 items-center rounded-md px-3 text-slate-700 hover:bg-slate-100"
                                }
                                key={item.href}
                                to={item.href}
                            >
                    ...
                    <div className="hidden items-center gap-3 md:flex">
                        ...
                        <button className="sf-button-secondary min-h-11" onClick={onLogout}>
                            Sair
                        </button>
                    </div>
                    <button
                        ...
                        className="min-h-11 min-w-11 rounded-md md:hidden"
            ...
            {mobileOpen ? (
                <div className="border-b border-slate-200 bg-white md:hidden" id="mobile-navigation" ref={mobileMenuRef}>
                    <nav aria-label="Navegação mobile" className="mx-auto grid max-w-6xl gap-1 px-4 py-3">
                        {navigation.map((item) => (
                            <NavLink className="flex min-h-11 items-center rounded-md px-3" key={item.href} onClick={() => setMobileOpen(false)} to={item.href}>
                                {item.label}
                            </NavLink>
                        ))}
                    </nav>
            ...
            <main className="mx-auto max-w-6xl px-4 py-6" id="main-content">{children}</main>
        </div>
    );
}
```
...
`AppShell` consome `navigation.ts` e deixa o React Router calcular o link ativo. `NavLink` aplica `aria-current="page"`; a shell não lê URLs diretamente, não guarda sessão e não decide autorização. O disclosure fica no fluxo do documento, abaixo do header, pelo que não cobre o conteúdo nem cria overflow horizontal.
...
2. Ficheiros envolvidos:
    - REVER: `apps/web/src/routes/AppRouter.tsx`
    - REVER: `apps/web/src/components/layout/navigation.ts`
    - LOCALIZAÇÃO: árvore `Routes`, `RoleGuard` e lista `navigationItems`.

3. Instruções do que fazer.

Compara cada `href` de `navigationItems` com os `Route path` declarados. Mantém apenas rotas reais e confirma que cada ramo por papel está sob o `RoleGuard` correspondente.
...
Pesquisa cada caminho no router. Expected result: todos os caminhos principais aparecem em `Route`, uma rota proibida mostra 403 sem montar a página, e um caminho desconhecido mostra 404 em vez de cair silenciosamente no dashboard.
