...
## Contexto do BK
- Entrega alvo: `Validação completa de formulários antes de submissão.` com rastreabilidade direta para `RNF06`.
...
- [ ] Sei explicar como `RNF06` se traduz em comportamento implementável.
...
### Passos
1. Confirmar no backlog e na matriz o escopo de `BK-MF5-08` e do requisito `RNF06`.
...
export function validarFormulario(state: FormState) {
  ...
  return { bkId: 'BK-MF5-08', req: 'RNF06', valido: erros.length === 0, erros };
}
```

Cria feedback imediato e determinístico no fluxo de UI.
- Requisitos alvo deste BK: `RNF06`.
