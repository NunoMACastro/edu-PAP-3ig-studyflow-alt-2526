...
- [ ] Sei explicar como `RNF14` se traduz em comportamento implementável.
...
export function validarRateLimit(tentativasMinuto: number, limite: number) {
  ...
  return { bkId: 'BK-MF6-04', req: 'RNF14', ok: true };
}
```

Aplica proteção ativa no perímetro do endpoint crítico.
- Requisitos alvo deste BK: `RNF14`.
