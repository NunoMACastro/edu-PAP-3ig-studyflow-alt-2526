...
- [ ] Sei explicar como `RNF15` se traduz em comportamento implementável.
...
export function validarRateLimit(tentativasMinuto: number, limite: number) {
  ...
  return { bkId: 'BK-MF6-05', req: 'RNF15', ok: true };
}
```

Aplica proteção ativa no perímetro do endpoint crítico.
- Requisitos alvo deste BK: `RNF15`.
