...
## Contexto do BK
- Entrega alvo: `Processamento de documentos em sandbox seguro.` com rastreabilidade direta para `RNF18`.
...
- [ ] Sei explicar como `RNF18` se traduz em comportamento implementável.
...
### Passos
1. Confirmar no backlog e na matriz o escopo de `BK-MF6-08` e do requisito `RNF18`.
...
export function validarRateLimit(tentativasMinuto: number, limite: number) {
  ...
  return { bkId: 'BK-MF6-08', req: 'RNF18', ok: true };
}
```

Aplica proteção ativa no perímetro do endpoint crítico.
- Requisitos alvo deste BK: `RNF18`.
