...
## Contexto do BK
- Entrega alvo: `Guardrails obrigatórios na IA.` com rastreabilidade direta para `RNF19`.
...
- [ ] Sei explicar como `RNF19` se traduz em comportamento implementável.
...
### Passos
1. Confirmar no backlog e na matriz o escopo de `BK-MF6-09` e do requisito `RNF19`.
...
export function validarRateLimit(tentativasMinuto: number, limite: number) {
  ...
  return { bkId: 'BK-MF6-09', req: 'RNF19', ok: true };
}
```

Aplica proteção ativa no perímetro do endpoint crítico.
- Requisitos alvo deste BK: `RNF19`.
