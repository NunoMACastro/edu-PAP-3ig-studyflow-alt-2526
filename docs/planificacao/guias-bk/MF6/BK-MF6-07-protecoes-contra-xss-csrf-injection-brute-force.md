...
## Contexto do BK
- Entrega alvo: `Proteções contra XSS, CSRF, Injection, brute force.` com rastreabilidade direta para `RNF17`.
...
- [ ] Sei explicar como `RNF17` se traduz em comportamento implementável.
...
### Passos
1. Confirmar no backlog e na matriz o escopo de `BK-MF6-07` e do requisito `RNF17`.
...
export function validarRateLimit(tentativasMinuto: number, limite: number) {
  ...
  return { bkId: 'BK-MF6-07', req: 'RNF17', ok: true };
}
```

Aplica proteção ativa no perímetro do endpoint crítico.
- Requisitos alvo deste BK: `RNF17`.
