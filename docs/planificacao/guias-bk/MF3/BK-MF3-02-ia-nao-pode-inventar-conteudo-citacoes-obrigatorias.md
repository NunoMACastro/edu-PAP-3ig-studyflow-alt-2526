...
Neste BK vais implementar respostas IA com citações obrigatórias. O guia parte dos contratos canónicos de RF38, da sequência MF0-MF2 e dos BKs que desbloqueiam este requisito.

#### Importância

Este BK transforma o requisito RF38 numa entrega copiável e testável. A funcionalidade fica no backend, com validação, sessão autenticada e resposta tipada para o frontend. Assim, o aluno percebe o domínio StudyFlow antes de escrever código e não precisa de adivinhar services, DTOs ou endpoints.

#### Scope-in

- Perguntar à IA com fontes indexadas.
- Bloquear sem fontes.
- Guardar resposta, pergunta e citações.
...
- O BK apresenta endpoint `POST /api/ai/source-grounded-answers`, DTO, backend, frontend, validações e handoff para `BK-MF3-03`.
...
- O endpoint `POST /api/ai/source-grounded-answers` é DERIVADO como contrato técnico mínimo para cumprir RF38 sem contrariar os documentos canónicos.
...
- `BK-MF2-07` com `MaterialIndexService.findDoneJob`.
- `BK-MF0-12` com `AiModule` e `GovernedAiExecutionService`.
...
- Uma fonte processável é um material indexado com texto extraído e estado `DONE`.
- Citação é a ligação entre a resposta e o excerto usado para a justificar.
- O backend bloqueia a resposta se não houver fontes, porque a IA não pode completar factos por suposição.
- O provider IA recebe apenas excertos autorizados e instruções de resposta fundamentada.
...
- Endpoint: `POST /api/ai/source-grounded-answers`.
- Backend: `apps/api/src/modules/source-grounded-ai`.
- Frontend: `apps/web/src/features/source-grounded-ai`.
- DTO principal: `AskSourceGroundedAiDto`.
- Service principal: `SourceGroundedAiService`.
- Controller principal: `SourceGroundedAiController`.
- Módulo principal: `SourceGroundedAiModule`.
- Handoff: `BK-MF3-03`.

#### Ficheiros a criar/editar/rever

- CRIAR: `apps/api/src/modules/source-grounded-ai/dto/ask-source-grounded-ai.dto.ts`
- CRIAR: `apps/api/src/modules/source-grounded-ai/schemas/source-grounded-ai-answer.schema.ts`
- CRIAR: `apps/api/src/modules/source-grounded-ai/source-grounded-ai.service.ts`
- CRIAR: `apps/api/src/modules/source-grounded-ai/source-grounded-ai.controller.ts`
- CRIAR: `apps/api/src/modules/source-grounded-ai/source-grounded-ai.module.ts`
- CRIAR: `apps/web/src/features/source-grounded-ai/ask-source-grounded-ai.ts`
- CRIAR: `apps/web/src/features/source-grounded-ai/source-grounded-ai-panel.tsx`
...
6. Validação do passo.
   Envia para `POST /api/ai/source-grounded-answers` um body vazio e confirma que a validação devolve `400`.
            ...
            ({ result: providerResult } = await this.aiExecution.execute({
                ...
                validateResult: (value) => {
                    if (!value || typeof value !== "object") {
                        throw new TypeError("Resposta com fontes inválida.");
                    }
                },
            }));
        } catch {
...
1. Objetivo funcional do passo no contexto da app.
   Ligar `POST /api/ai/source-grounded-answers` ao service sem colocar regras sensíveis no controller.
2. Ficheiros envolvidos:
   - CRIAR: `apps/api/src/modules/source-grounded-ai/source-grounded-ai.controller.ts`
...
6. Validação do passo.
   Faz um pedido sem cookie para `POST /api/ai/source-grounded-answers` e confirma `401 Unauthorized`.
7. Cenário negativo/erro esperado.
...
3. Instruções do que fazer.
   Cria o módulo e adiciona `SourceGroundedAiModule` à lista de imports do AppModule, preservando os módulos existentes.
4. Código completo, correto e integrado com a app final.
...
- Pedido válido para `POST /api/ai/source-grounded-answers` devolve `201 Created` com resposta e citações.
...
- O endpoint `POST /api/ai/source-grounded-answers` está alinhado entre controller e cliente frontend.
...
- O handoff para `BK-MF3-03` fica claro.
...
```bash
curl -i -X POST http://localhost:3000/api/ai/source-grounded-answers \
  -H 'Content-Type: application/json' \
  -d '{ "question": "Resume a fotossíntese.", "sourceJobIds": ["job_123"] }'
```
...
- Referência ao requisito `RF38` e ao próximo BK `BK-MF3-03`.

#### Handoff

- Este BK entrega `SourceGroundedAiModule`, `SourceGroundedAiService`, `SourceGroundedAiController` e cliente frontend tipado.
- O próximo BK é `BK-MF3-03`.
