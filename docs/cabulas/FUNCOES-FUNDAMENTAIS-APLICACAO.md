...
- `TeacherClassAiVoicePage({ classId })` (exportada; função) - Página de configuração da voz textual base da turma. Entradas: `props`: Propriedades recebidas pelo componente React; concentram os dados e callbacks necessários para renderizar a UI. Devolve: Elemento React pronto a ser renderizado pela página ou rota atual.
- `TeacherClassAiVoicePage.handleSubmit(event)` (interna; função) - Trata a acao do utilizador e sincroniza o estado da interface. Entradas: `event`: Evento da interface usado para impedir o comportamento padrão e recolher dados do formulário. Devolve: Não devolve payload; termina quando os efeitos locais ou remotos ficam concluídos.
...
- `TeacherVoiceForm({ title, voice, rulesText, error, success, isSubjectOverride, onVoiceChange, onRulesChange, onSubmit, onDeleteOverride, })` (top-level; função) - Renderiza a experiência de páginas React, ligando estado local, dados remotos e ações do utilizador num componente reutilizável. Entradas: `props`: Propriedades recebidas pelo componente React; concentram os dados e callbacks necessários para renderizar a UI. Devolve: Elemento React pronto a ser renderizado pela página ou rota atual.
